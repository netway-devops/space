...
class RvapikeyMgr extends OtherModule
{
	public function showapikey()
	{
		...
		if (is_writable(HBFDIR_LIBS . 'RvLibs/key.pem')) {
			$handle =fopen(HBFDIR_LIBS . 'RvLibs/key.pem', 'w+');
			fwrite($handle, $aResData['publickey']);
			fclose($handle);
		} else {
	...
	public function user_showapikey()
	{
		...
		if (count($aResKeyInfo) < 1) {
			...
			$aAPIKeyInfo['email'] = $_SESSION['AppSettings']['login']['email'];
		} else {
			$aAPIKeyInfo = $aResKeyInfo[0];
		}
		$aResData = array();
		...
		$aResData['email'] = $aAPIKeyInfo['email'];
		return $aResData;
	}
