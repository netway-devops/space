...
class LN_WRIKE {
    ...
    public static function singleton($secret, $consumer, $token, $token_secret){
        static $inst = null;
        if ($inst === null) {
            $inst = new LN_WRIKE($secret, $consumer, $token, $token_secret);
        }
        return $inst;
    }
	...
	public function getRequestToken() {
		...
		$callback_url = "https://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["REQUEST_URI"]) . "/authorization.php?oauth_token=" . $token . "&oauth_token_secret=" . $token_secret;
	...
	public function getAccessToken($token, $secret) {
		...
		$_url = "http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["REQUEST_URI"]) . "/authorization.php?access_token=" . $access_token . "&access_token_secret=" . $access_token_secret;
...
*/
	 ...
	 * 
	 */
	public function getTask($id = false, $fields = false, $asyncKey = false) {
	 ...
	 * 
	 */
	public function getTasks($fields = false, $limit = false, $offset = false, $asyncKey = false) {
		$consumer = $this->consumer;
	 ...
	 * 
	 */
	public function addTask($title = false, $description = false, $status = false, $importance = false, $startDate = false, $dueDate = false, $duration = false, $responsibleUsers = false, $sharedUsers = false, $parents = false, $account = false, $asyncKey = false) {
	 ...
	 * 
	 */
	public function updateTask($id = false, $title = false, $description = false, $status = false, $importance = false, $startDate = false, $dueDate = false, $duration = false, $responsibleUsers = false, $sharedUsers = false, $parents = false, $asyncKey = false) {
		...
		if ( $title ) 				$params['title'] 			= $title;
		if ( $description ) 		$params['description'] 		= $description;
		if ( $status!=false || $status==0) 	    $params['status'] 			= $status;
		if ( $importance ) 			$params['importance'] 		= $importance;
		if ( $startDate ) 			$params['startDate'] 		= $startDate;
		if ( $dueDate ) 			$params['dueDate'] 			= $dueDate;
		if ( $duration ) 			$params['duration'] 		= $duration;
	 ...
	 * 
	 */
	public function deleteTask($ids = false, $asyncKey = false) {
	 ...
	 * 
	 */
	public function addComment($taskId = false, $text = false, $asyncKey = false) {
	 ...
	 * 
	 */
	public function updateComment($id = false, $text = false, $asyncKey = false) {
