...
try {
    $db   = new PDO ("mysql:host=" . $server_ip . ";dbname=" . $dbname, $username, $pwd);  
} catch (PDOException $e) {
    //$res['result']    = false;
   // $res['txt']   =  'case error connect hostbill'.$e."\n";
}

$outputData = fopen ( "/home/rvglobal/public_html/MYPayPalReturn.txt", "a+");
...
if (preg_match('/VERIFIED/',$output)) {
    ...
    $result = $db->query("
        ...
        FROM 
            mb_invoice_log 
        WHERE 
            invoice_id ='".$item_number."'")->fetch();
     if ($result) {
        $my_cp_id       = $result[cp_id];
        $my_client_id   = $result[client_id];
        $my_inv_id      = $result[invoice_id];
     } else {
        $result2        = $db->query("SELECT * FROM mb_client_invoice WHERE invoice_id ='".$item_number."'")->fetch();
    ...
    $in_mb_paypal = $db->query("
            ...
            VALUES 
                ('".$my_cp_id."','".$my_client_id."','".$txn_type."','".$post_string."')");
...
// Insert all data to license table 
    ...
    if ($txn_type == 'subscr_payment' || $txn_type == 'web_accept') {
        /// RV Global Soft - Stop 
        if (preg_match('/failed/i',$payment_status)) {
            paypal_ipn_failed($payment_status,$item_number,$txn_id,$date_format,$subscr_id,$pending_reason,$outputData,$pp_debug,$mc_gross);
        } else if (preg_match('/denied/i',$payment_status)) {
            paypal_ipn_failed($payment_status,$item_number,$txn_id,$date_format,$subscr_id,$pending_reason,$outputData,$pp_debug,$mc_gross);
        } else if (preg_match('/pending/i',$payment_status)) {
            paypal_ipn_failed($payment_status,$item_number,$txn_id,$date_format,$subscr_id,$pending_reason,$outputData,$pp_debug,$mc_gross);
        } else if (preg_match('/Refunded/i',$payment_status)) {
            paypal_ipn_failed($payment_status,$item_number,$txn_id,$date_format,$subscr_id,$pending_reason,$outputData,$pp_debug,$mc_gross);
        } else if (preg_match('/Reversed/i',$payment_status)) {
            paypal_ipn_failed($payment_status,$item_number,$txn_id,$date_format,$subscr_id,$pending_reason,$outputData,$pp_debug,$mc_gross);
        } else if (preg_match('/Completed/i',$payment_status)) {
...
{
    global $db;
    $subject    = 'NEW '. date('Y-m-d') .' billing.rvglobalsoft.com : Paypal';
    $headers    = 'From: admin@rvglobalsoft.com' . "\r\n" .
    'Reply-To: rvglobalsoft.com' . "\r\n" .
    'X-Mailer: PHP/' . phpversion();
    ...
    @mail('prasit@netway.co.th', $subject . ': debug', $message, $headers);
    ...
    $resgethbacc = $db->query("
        ...
        AND c.cp_id = '".$cpid."'")->fetch();
    if (isset($resgethbacc['hb_acc']) && $resgethbacc['hb_acc'] != '' && $resgethbacc['hb_acc'] != 0) {
        $hb_acc_id = $resgethbacc['hb_acc'];

        $sqlgetmail = $db->query("
            SELECT c.email FROM hb_client_access c,hb_accounts a WHERE c.id=a.client_id and a.id ='".$hb_acc_id."'
            ")->fetch();    
    ...
    // 1. connect กับ database ของ hostbill สำเร้จ 
        $iparrow = '173.192.164.229';//$_SERVER['SERVER_ADDR'];

    //  1.2 หาใบ invoice ที่ยังไม่ได้จ่าย

        $getinv = $db->query("SELECT i.id
            FROM `hb_invoices` i, hb_invoice_items ii
            WHERE i.id = ii.invoice_id AND ii.item_id ='".$hb_acc_id."' AND i.status =  'Unpaid'")->fetch();
        $date   = date('Y-m-d H:i:s');

        $getapi = $db->query("SELECT aa.* FROM hb_api_access aa WHERE aa.ip ='".$iparrow."'")->fetch();
        ...
        if (isset($getapi['api_id']) && isset($getapi['api_key']) && isset($getinv['id'])) {
            ...
            @mail('panya@netway.co.th', 'debug: ' . $inv_id , 'msg: ' . print_r($return) , $headers);
...
EOF;
    ...
    @mail('panya@netway.co.th', $subject, $message, $headers);
    @mail('patcharapan@rvglobalsoft.com', $subject, $message, $headers);
    @mail('sirishom@rvglobalsoft.com', $subject, $message, $headers);
...
function pp_getClientID($item_number)
{
       global $db;
          if (isset($item_number)) {
            ...
            $this_client_id = $db->query("SELECT `client_id` FROM `mb_client_invoice` WHERE `invoice_id` = '".$item_number."'")->fetch();
...
function log_event ($this_client_id,$log_comments,$status){
    ...
    $insertlog = $db->query("INSERT INTO mb_log_event (`id`, `mb_client_id`, `log_comment`, `status`) 
     VALUES (NULL, '".$this_client_id."', '".$log_comments."','".$status."')");
