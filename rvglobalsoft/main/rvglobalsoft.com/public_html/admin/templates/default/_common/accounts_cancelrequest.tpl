{if $cancel_request}
    <div style="{if $cancel_request.disabled}border: 1px solid #CCCCCC; background: #DDDDDD;{else}{/if}" class="cancellation">
        ...
        {if !$cancel_request.disabled}<input type="{if isset($accounts.ssl_id) && $accounts.ssl_id}button{else}submit{/if}" name="cancellation" value="{$lang.CancelAccount}" onclick="{if isset($accounts.ssl_id) && $accounts.ssl_id}alert('For SSL products, please click Terminate button on \'Available actions\' tab.');{else}return confirm('{$lang.confirm_cancel_account}'){/if}" style="margin: 5px; font-weight: bold;" class="btn btn-danger btn-sm">{/if}
