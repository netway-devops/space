...
include($templatePath . 'ajax.emailvariables.tpl.php');
{/php}
{if $variables}
    <div class="well">
        <div class="fo"></div>
        ...
        <div class="form-group">
            ...
            <br />
            <select name="availvarsxxx" id="availvarsxxx" class="inpxxx" onchange="$('.availvarsDisplay').val(this.value).parent().show();/*loadvar(this);*/">
                    ...
                    {else}
                        <optgroup label="{if $lang[$var_key]}{$lang[$var_key]}{else}{$var_key}{/if}">
                            {foreach from=$var item=var_desc key=var_name}
                                <option value="{ldelim}${if $var_key != 'other'}{$var_key}.{/if}{$var_name}{rdelim}">{ldelim}${if $var_key != 'other'}{$var_key}.{/if}{$var_name}{rdelim} &nbsp;&nbsp;&nbsp; {if $lang[$var_desc]}{$lang[$var_desc]}{else}{$var_desc}{/if} </option>
                            {/foreach}
                        </optgroup>
                    {/if}
                {/foreach}
        ...
        <label style="display:none;"> &nbsp; สามารถคัดลอกตัวแปร &nbsp; <input type="text" class="availvarsDisplay" value="" readonly="readonly" size="40" style="height:30px;" /> &nbsp; ไปวางในแบบฟอร์ม</label></td>
