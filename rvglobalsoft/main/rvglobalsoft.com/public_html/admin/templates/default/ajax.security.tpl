...
{elseif $banned_ips}
    {foreach from=$banned_ips item=ban_ip}
        <tr {if $highlight == $ban_ip.id}class="checkedRow"{/if}>
            ...
            <td>{if $ban_ip.expires == '2999-01-01 00:00:00'}Never{else}{$ban_ip.expires|date_format:'%d %b %Y'}{/if}</td>
            <td>{$ban_ip.reason}</td>
...
{elseif $banned_emails}
    {foreach from=$banned_emails item=ban_mail}
        <tr>
            ...
            <td>{if $ban_mail.expires == '2999-01-01 00:00:00'}{$lang.never}{else}{$ban_mail.expires|date_format:'%d %b %Y'}{/if}</td>
