function ConfirmInvoiceEdit(callback) {
    return void 0 != typeof window.EditWarning && window.EditWarning ? void bootbox.confirm("This invoice has been issued and is locked                    for edit, are you sure you want to change it?", function(result) {
        return result ? (window.EditWarning = !1, callback(), !0) : !0
    }) : callback()
}
...
function toggleStatusActions() {
    ...
    if (status) {
        ...
        var statusLo = status.toLowerCase(),
            acts = $(".act-status");
        console.log(statusLo), acts.filter('[class*="off-"]').removeClass("disabled").filter(".off-" + statusLo).addClass("disabled"), acts.filter('[class*="on-"]').addClass("disabled").filter(".on-" + statusLo).removeClass("disabled")
    }
}
$(function() {
    ...
    function FormSubmit(e) {
        if (e.preventDefault(), 0 == $("#old_client_id").val()) return !0;
        ...
        newitem ? $("#itemsform").addClass("wrap-loading") : total.addClass("wrap-loading"), $.post(cmdUrl, form, function(data) {
            ...
            if (linebody.replaceWith(newlinebody), focus.each(function() {
                    ...
                    var old = newlinebody.find("#" + tr.attr("id")),
                        line = tr.data("line"),
                        inserted = !1;
                    return old.length ? void old.replaceWith(tr.clone()) : (newlinebody.children().each(function() {
                        return $(this).data("line") > line ? (tr.clone().insertBefore(this), inserted = !0, !1) : void 0
                    }), void(inserted || newlinebody.append(tr.clone())))
    ...
    function checkrecurring() {
        ...
        return "1" == recurring.val() && "0" != client && startDate == RcStartDate && startDate != oldStartDate ? confirm(lang.invoicerecurrnow) : !0
    }
    ...
    function hash_action(e) {
        ...
        switch (action) {
            case "save":
                return console.log("save"), ConfirmInvoiceEdit(function() {
                    var id = $("input[name=invoice_id]").val(),
                        form = $.extend(!0, $("#detailsform").serializeForm(), $("#itemsform").serializeForm());
    ...
    invoicebody.on("click", "#invoice-actions a, #invoice-queue a", hash_action), invoicebody.on("mouseenter", ".editor-line", function() {
    ...
    }), invoicebody.on("change", ".invitem", FormSubmit), invoicebody.on("click", ".invitem2", FormSubmit), invoicebody.on("change", "#detailsform input, #detailsform select", FormSubmit), invoicebody.on("click", ".tdetail a", ToggleEditMode), invoicebody.on("mouseenter mouseleave click", ".livemode", function(e) {
        var self = $(this);
        ...
        return index > aindex ? !1 : (navs.removeClass("active"), self.parent().addClass("active"), steps.hide().eq(index).show(), !1)
