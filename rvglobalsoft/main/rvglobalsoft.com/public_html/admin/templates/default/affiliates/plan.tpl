<form action="" method="post">
    ...
    <div class="lighterblue2">
        <table border="0" cellpadding="6" cellspacing="0" width="100%">
            ...
            <tr>
                <td align="right"><strong>Limit this commission plan to affiliates in group:</strong></td>
                <td>
                    <div class="row">
                        <div class="col-md-6">
                            <select multiple="multiple" name="applicable_client_groups[]"  class="form-control" id="applicable_client_groups"  style="height: 150px;">
                                <option value="all" {if $commision.applicable_client_groups.all == 'all'}selected="selected"{/if}>Apply to all client groups</option>
                                {foreach from=$clientgroups item=group}
                                    <option value="{$group.id}" {if $commision.applicable_client_groups[$group.id]}selected="selected"{/if}>{$group.name}</option>
                                {/foreach}
                            </select>
                        </div>
                    </div>
                ...
                <td>
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <select multiple="multiple" name="applicable_products[]"  class="form-control" id="applicable_products"  style="height: 300px;">
                                {foreach from=$products item=c}
                                    <option value="p{$c.category_id}" data-id="{$c.category_id}" data-category="true" {if $commision.applicable_products.products[$c.category_id]}selected="selected"{/if}>{$lang.Category}: {$c.category_name}</option>
                                    {foreach from=$c.products item=i}
                                        <option value="p{$i.id}" data-category="false" data-parent="{$c.category_id}" {if $commision.applicable_products.products[$i.id]}selected="selected"{/if} style="padding-left: 20px;">{$i.catname}: {$i.name}</option>
                                    {/foreach}
                                {/foreach}
                        ...
                        <div class="col-md-6">
                            ...
                            <select multiple="multiple" name="applicable_cycles[]"  id="cycles" class="form-control" style="height: 300px;">
