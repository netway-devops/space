...
include($templatePath . 'configuration/cron.tpl.php');
{/php}
...
{if $tasks}
    ...
    <div id="taskexec" style="display:none">
        ...
        {assign var="taskName" value=$profile.task}
            <div class="box box-primary no-padding" >
                ...
                <div class="box-body">
                    ...
                    {/if}
                    <tr id="task-{$task.task|replace:':':'_'}" class="{if $smarty.foreach.fr.index%2==0}even{/if} havecontrols {if $task.status==0}running{elseif $task.count>2}failed{/if}" valign="top">
                        <td>
                            {$taskname}
                            {if $task.count>2}
                            ...
                            {/if}
                            {if isset($aTaskSQLs.$taskName)}<span onclick="$(this).children().css('display','');"><u class="editbtn editgray" style="cursor:pointer;">Info</u><pre style="display:none;">{$aTaskSQLs.$taskName}</pre></span>{/if}
                        </td>
                        <td>
