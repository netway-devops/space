...
include($templatePath . 'ajax.stats.tpl.php');
{/php}

<script type="text/javascript" src="{$template_dir}js/graph/raphael-min.js"></script>
<script type="text/javascript" src="{$template_dir}js/graph/g.raphael.js"></script>
<script type="text/javascript" src="{$template_dir}js/graph/g.bar.js"></script>
...
{literal}
    ...
    <script type="text/javascript">
        ...
        {/literal}
        {if $chart || $mchart }
        var data = {if $chart && $chart!='null' && $chart!='[]'}[{$chart}]{elseif $mchart && $mchart!='null' && $mchart!='[]'}{$mchart}{else}{literal}[]{/literal}{/if};;
                
        var ticks = {if $tick && $tick!='null'}{$tick}{else}[]{/if};;
        var total = {if $total && $total!='null' && $total!='[]'}{$total}{else}{literal}{value:0, prefix:'', sufix:''}{/literal}{/if};;
        var bar = {if $bar && $bar!='null' && $bar!='[]'}{$bar}{else}{literal}{prefix:'', sufix:''}{/literal}{/if};;
        {literal}
        ...
        function isEmptyData(data){
            ...
            if(data.length > 0){
                for(var i=0;i<data.length;i++){
                    if(typeof data[i] == 'number' || typeof data[i] == 'object'){
                        s++
                    }
                }
            }
        ...
        var xoff = {/literal}{if $graphxoff}{$graphxoff}{else}100{/if}{literal},
            yoff = {/literal}{if $graphyoff}{$graphyoff}{else}0{/if}{literal},
            boff = 30,
            gHeight = $("#graph1div").height(),
            gWidth = $("#graph1div").width(),
            barheight = Math.floor((gHeight-yoff) / (ticks.length * (100 + 20) + 20) * 100),
            bargutter = Math.floor(barheight * 20 / 100),
            labelheight = 3;
                
        window.onload = function() {
                    graph = Raphael("graph1div", gWidth, gHeight);
                    for(var w = 0; w<ticks.length; w++){
                    var wt = graph.g.text(gWidth, gHeight, ticks[w]);
                    if(wt.getBBox().width+20 > xoff){
                        xoff = wt.getBBox().width+20;
                    }
                }
                graph.clear();
                if(isEmptyData(data) > 0){
                    var res = graph.g.hbarchart(xoff, yoff, (gWidth-xoff*1.8), gHeight-boff, data);
                    res.hoverColumn(function(a, b) { 
                        this.flag = graph.g.popup(a.layerX, a.layerY, bar.prefix+this.bar.value+bar.sufix);
                    }, function() {
                        this.flag.animate({opacity: 0}, 300, ">", function () {this.remove();});
                    });
                    var tickwidth = 0;
                    for(var i=0; i < res.bars.length; i++){
                        $.each(res.bars[i], function(index){
                            var mbar = bar;
                            if (typeof bar == 'object' && typeof bar[i] == 'object'){
                                mbar = bar[i];
                            }
                            if(!total.fixedvalue)
                                total.value += this.value;
                            var at = graph.g.text(this.x+5, this.y, mbar.prefix+this.value+mbar.sufix);
                                at.attr( 'text-anchor','start' );
                            var tickid = (index * data.length) +i;
                            if(ticks[tickid] !== undefined){
                                if(typeof ticks[tickid] == 'object'){
                                    var lab1 = graph.g.text(10, this.y, ticks[tickid][0]);
                                    lab1.attr( 'text-anchor','start' );
                                    var lab2 = graph.g.text(xoff-5, this.y, ticks[tickid][1]);
                                    lab2.attr( 'text-anchor','end' );
                                }else{
                                    var lab = graph.g.text(xoff-5, this.y, ticks[tickid]);
                                    lab.attr( 'text-anchor','end' );
                                }
                            }  
                        });
                    }
                    graph.path('M'+0+' '+(gHeight-boff)+' H'+gWidth);
                    graph.text(gWidth, gHeight-boff/2, total.prefix+(Math.round(total.value*100)/100)+total.sufix).attr( {'text-anchor':'end', font: "12px Arial, sans-serif"});
                }else{
                    graph.text(gWidth/2, (gHeight/2)-18, 'No data to display').attr( {'text-anchor':'middle', font: "18px Arial, sans-serif"});
                    $('svg').hide();
                }       
            }
        {/literal}{/if}{*END IF GRAPHS*}{literal}
        ...
        <div class="list-1">
            <ul>
                ...
                <li class="datetab last">
                    <a href="#"><span>{$dateFrom|date_format:'%d %b %Y'} - {$dateTo|date_format:'%d %b %Y'}</span><span class="picker"></span></a>
                </li>
                {if $monetary}
            ...
            <table style="width: 100%" cellspacing="0" cellpadding="5" class="table glike hover">
                ...
                {foreach from=$data.items item=item key=itemkey}
                    ...
                    *}
                        ...
                        <td>{$item.date|date_format:'%d %b %Y'}</td>
                        {foreach from=$data.timeline item=items key=period}
