...
<div id="privileges" {if $client.status == 'Closed'}style="display: none;"{/if}>
    <div style="padding:5px 20px 0">
        {foreach from=$privilages item=privs key=group}
            ...
            {/if}{/foreach}
            {if $displaythis!=1}{continue}
            {/if}
            <h3 class="{$privs._type}_h3 {if $privs._type}privs_items{/if}"><input type="checkbox" class="privilege " id="{$group}_main" onclick="cUnc(this, '{$group}')"/>
                {if $privs._type}
                    ...
                    <tr>
                        ...
                        <td width="33%">
                            ...
                            {else}
                                ...
                                <input type="checkbox" class="privilege {$group}" id="{$group}_{$priv}" value="1"
                                       name="privileges[{$group}][{$priv}]"
                                       {if $submit.privileges.$group.$priv}checked="checked"{/if} />
                                <label for="{$group}_{$priv}" {if $privopt.important}style="color:red"{/if}>
