                    ...
                    if (resp.macro.errors != undefined){
                        ...
                        if(resp.macro.tags != undefined && resp.macro.tags != null && resp.macro.tags.length){
                            $('#tagsCont').each(function () {
                                ...
                                $(this).data('hbtags').insertTags(resp.macro.tags)
                            });
                        }
                        if(resp.macro.tags_rem != undefined && resp.macro.tags_rem != null && resp.macro.tags_rem.length){
                            $('#tagsCont').each(function () {
                                //create ticket interface does not have #tagsCont
                                $(this).data('hbtags').removeTags(resp.macro.tags_rem);
                            });
                        }
...
{if $action=='gettop'}
    <div id="replybrocontainer">
        ...
        {/if}
        <br />
        <fieldset class="gbox1">
            <legend><strong>ค้นหา KB จาก google docs</strong></legend>
            <p>
                <form onsubmit="javascript: window.open(this.action+document.getElementById('searchGoogleDocs').value,'gdocs'); return false;" action="https://drive.google.com/a/rvglobalsoft.com/?tab=mo&pli=1#advanced-search?view=2&filter=documents&q=" method="get" target="gdocs" style="margon:0px; padding:0px;">
                <input type="text" id="searchGoogleDocs" name="q" class="notes_field notes_changed" style="width:94%" />
                </form>
            </p>
        </fieldset>
    </div>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-8">
                    <div class="form-group">
                        ...
                        <select name="category_id" id="catpicker" class="form-control">
                            {foreach from=$categories item=cat}
                                <option value="{$cat.id}" {if $reply.category_id==$cat.id || $parent_category_id == $cat.id}selected="selected"{/if}>&mdash;{section loop=$cat.depth name=sect}&mdash;{/section} {$cat.name} </option>
                            {/foreach}
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Reply</label>
                        <textarea class="form-control" rows="12"  name="reply">{$reply.reply}</textarea>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Note</label>
                        <textarea class="form-control" rows="12"  name="admin_note">{$reply.admin_note}</textarea>
                    </div>
                    <div class="form-group">
