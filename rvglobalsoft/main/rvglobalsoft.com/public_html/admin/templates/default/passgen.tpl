<table border="0" cellspacing="0" cellpadding="0" width="100%" id="content_tb">
	...
	<tr>
		<td class="leftNav">
			{if isset($admindata.access.menuGeneralconfig)}<a href="?cmd=configuration"  class="tstyled">{$lang.generalsettings}</a>{/if}
			...
			{if isset($admindata.access.menuSecuritysettings)}<a href="?cmd=security"  class="tstyled  {if $action == 'default'}selected{/if}">{$lang.securitysettings}</a>{/if}
