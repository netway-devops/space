...
{assign var="invoiceId" value=$invoice.id}
    <tr data-flags="{$invoice.flags}">
        ...
        {if $currentlist!='recurring'}
            ...
            <td>
                {if $invoice.client_id}
                     <a href="?cmd=clients&action=show&id={$invoice.client_id}" class="isclient isclient-{$invoice.group_id}">{$invoice.firstname} {$invoice.lastname}</a>
                {else}
                    <span style="font-style: italic">{$lang.no_clients_attached}</span>
                {/if}
                <small>{$aInvoiceServices[$invoiceId]}</small>
            </td>
            <td>{$invoice.date|date_format:'%d %b %Y'}</td>
        ...
        {else}
            ...
            <td><a href="?cmd=clients&action=show&id={$invoice.client_id}" class="isclient isclient-{$invoice.group_id}">{$invoice.firstname} {$invoice.lastname}</a></td>
            <td>{$invoice.grandtotal|price:$invoice.currency_id}</td>
            <td>{$invoice.start_from|date_format:'%d %b %Y'}</td>
            <td>
                {if $invoice.recstatus!='Stopped' &&  $invoice.next_invoice!='' && $invoice.next_invoice!='0000-00-00' && ($invoice.invoices_left || !$invoice.occurrences)}
                    {$invoice.next_invoice|date_format:'%d %b %Y'} ({if $invoice.invoices_left && $invoice.occurrences}{$invoice.invoices_left}
                    {else}&#8734;
                    {/if} {$lang.remaining})
                {else}-
                {/if}
