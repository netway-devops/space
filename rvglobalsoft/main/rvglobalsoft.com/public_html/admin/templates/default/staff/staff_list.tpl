<table width="100%" cellspacing="0" cellpadding="3" border="0" class="table glike">
    ...
    {foreach from=$admins item=ad}
        <tr class="product">
            <td>{$ad.id}</td>
           <td>
                <img src="{if $ad.profile_picture_url != ''}{$ad.profile_picture_url}{else}{$template_dir}/img/sblock.png{/if}" width="24" align="left" style="margin: 5px;" />
                <a href="?cmd=editadmins&action=administrator&id={$ad.id}">{$ad.firstname} {$ad.lastname}</a>
            </td>
            <td>{$ad.username}</td>
