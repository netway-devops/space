...
<!-- rv delete mail -->
 function deletemail(mail_id) {
    ...
    $.post('?cmd=rvglobalsoft_common&action=email_delete',{
        selected:mail_id
    }, function(data){
        var resp = parse_response(data);
        $('tr[dir='+mail_id+']').hide();
...
{elseif $action=='default'}
 ...
 function deletemail(frm) {
 	var param = $('#testform').serialize(); 
    $.post('?cmd=rvglobalsoft_common&action=email_delete',param, function(data){
        var resp = parse_response(data);
            ...
            <form action="" method="post" id="testform" ><input type="hidden" value="{$totalpages}" name="totalpages2" id="totalpages"/>
                <div class="blu">
                    <div class="left">
                        ...
                        <!-- rv ใช้สำหรับ ลบ mail เพราะไม่อยากให้ลูกค้าเห็น -->
                        <input type="button" name="delete_mail" value="delete mail" onclick="deletemail(this);" class="submiter" />
                    </div>
