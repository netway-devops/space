    ...
    <div id="formcontainer">
                        ...
                        <td class="conv_content" valign="top">
                                ...
                                {if !($field.options & 1)}
                                    <div class="form-group">
                                        ...
                                        <select name="type" class="form-control">
                                            <option value="All"
                                                    {if $field.type=='All'}selected="selected"{/if}>{$lang.All}</option>
                                            <option value="Private"
                                                    {if $field.type=='Private'}selected="selected"{/if}>{$lang.Private}</option>
                                            <option value="Company"
                                                    {if $field.type=='Company'}selected="selected"{/if}>{$lang.Company}</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        {if $field.id=='new'}
                            ...
                            <div class="tabb" style="display:none;overflow:visible">
                                ...
                                $field.code!='firstname' && $field.code!='lastname' && $field.code!='city' && $field.code!='state' && $field.code!='postcode'}
                                    <div class="checkbox">
                                        <label>
                                            <input id="check-uniquevalue" type="checkbox" name="options[]" value="2048"
        ...
        <script type="text/javascript">
            ...
            $('#field-type').on('change', function () {
                ...
                $('#form-placeholder').toggle(val == 'Input');

                if (val == 'File') {
                    $('#form-default-value').hide();
                    $('#form-extension').show();
                    $('#check-hidecontacts').prop('checked',true).closest('.checkbox').hide();
                    $('#form-expression').hide();
                } else if (val == 'Contact') {
                    $('#form-default-value').hide();
                    $('#form-extension').hide();
                    $('#check-hidecontacts').prop('checked',true).closest('.checkbox').hide();
                    $('#check-noregister').prop('checked',true).closest('.checkbox').hide();
                } else {
                    $('#form-default-value').show();
                    $('#form-extension').hide();
                    $('#check-hidecontacts').prop('checked',false).closest('.checkbox').show();
        ...
        <div class="blu">
            <div class="left menubar">
                <a onclick="addCustomFieldForm();
                        return false;" class="new_control" href="#"><span
                            class="addsth"><strong>{$lang.addnewfield}</strong></span></a>
                {if $premade}
                    <a onclick="$('#premadefields').toggle();
                        return false;" class="new_control" href="#"><span class="dwd"><strong>Premade field
                                profiles</strong></span></a>
                {/if}

                <a class="new_control" href="{$system_url}?cmd=signup" target="_blank"><span
                            class="zoom">{$lang.Preview}</span></a>
            </div>
            <div class="clear"></div>
        </div>
