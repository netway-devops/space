var solusvm = {
    ...
    lookforsliders : function() {
        ...
        $('.formchecker').click(function(){
            ...
            $(this).parents('span').eq(0).append(' <a href="#" onclick="return solusvm.updateOSList('+fid+')" class="editbtn orspace">Update template list from server</a>');
        });
    },
...
function filter_types(){ 
    $('[class^="opt_"]').hide(); 
    var type = $('#vpstype').val();

    $('.opt_'+type).show();

    $('.opt_'+type).each(function(){
        if($(this).parent().is('#server') && $('#server option:first').is(':selected'))
            return;
    ...
    var enabled = [];
    if($('[class^="opt_"] option', '#vpsplan, #server').length > 0){
        $('#vpstype option').each(function(){
            if(!$('.opt_'+$(this).val()+' option', '#vpsplan, #server').length){
            ...
            if(!$('#vpsplan .opt_'+$(this).val()).length && $('#vpsplan .opt_'+$(this).val()).length>0){
                    switch($(this).val()){
                    ...
                    $('#vpsplan').append('<optgroup label="'+label+'" class="opt_'+$(this).val()+'"><option value="">--none--</option></optgroup>');
                }
        });
    }
    if(!$('#vpstype').val() && enabled.length)
        $('#vpstype').val(enabled.pop())
