...
(function(jQuery) {
     ...
     $.VIP.Log = {
             logMessage: function(message, level) {
                 if (window.console) {
                     if (!document.all) {
                    	 console.debug($.VIP.Log.getLogLevelMsg(level) + ': ' + message);
                     }
                 }
                 return this;
             },
    		 ...
    		 connect: function(param) {
	             ...
	             option = {
	                     success: function (data) {
	 			        		...
	 			        		$.VIP.Log.logMessage('AJAX returns: '+ data, LOG_PRIORITY.DEBUG);
	                   ...
	                   error: function (xhr, ajaxOptions, thrownError) {
	                         	...
	                         	$('body').css({'cursor':'default'});
	                            $.VIP.Log.logMessage('AJAX connect error '+ xhr.status +': '+ xhr.statusText, LOG_PRIORITY.ERR);
	                             ...
	                             if (xhr.status == 500 || xhr.status == '500') {
	                            	 $.VIP.Log.logMessage('AJAX connect response text: ' + xhr.responseText, LOG_PRIORITY.ERR);
	                             ...
	                             if (typeof settings.callback.doError === 'function') {
	                            	 ...
	                            	 $.VIP.Ajax.doError(xhr, ajaxOptions, thrownError, defXHR);
	                             }
	                         }
	                 };
		             ...
		             if (param.match(/^[#|\.]/)) {
			            	$.VIP.Log.logMessage('Usage ajaxSubmit (jQuery form)', LOG_PRIORITY.DEBUG);
			                $(param).ajaxSubmit(option);
		             } else {
			               	$.VIP.Log.logMessage('Usage ajax (jQuery default)', LOG_PRIORITY.DEBUG);
			                option.data = param;
			                $.ajax(option);
		             }
