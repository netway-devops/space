...
class buycpanel_freervskinfreervsitebuilder extends HostingModule
{
	protected $description = 'Buy cPanel Free RVSkin Free RVsitebuilder';
	...
	*/
    ...
    // [TODO] ทำ function นี้ //added
    private function setRVsitebuilderModule()
    {
        require_once APPDIR_MODULES . "Hosting/rvsitebuilder_license/class.rvsitebuilder_license.php";
        $oRVsitebuilderLicense= new rvsitebuilder_license();
        $oRVsitebuilderLicense->connect(array());
        $oRVsitebuilderLicense->account_details = $this->account_details;
        $oRVsitebuilderLicense->account_config = $this->account_config;
        $oRVsitebuilderLicense->product_details = $this->product_details;
        $aServerConf = RVCPanelDao::singleton()->getDataServerTypeLinkCpanel($this->account_details['product_id']);
        if (!isset($oRVsitebuilderLicense->product_details['options'])) {
        	$oRVsitebuilderLicense->product_details['options'] = array();
        }
        $oRVsitebuilderLicense->product_details['options']['Server Type'] = $aServerConf[0]['server_type'];
        return $oRVsitebuilderLicense;
    }
	...
	public function Create()
	{
		if($this->validateRisk()){
            ...
            // [TODO] Add RVsitebuilder Licnese //added
			$rvsitebuilderRes = $cpanelRes = false;
			if($this->checkCreation()){
				$oRVsitebuilderLicense = $this->setRVsitebuilderModule();
				$rvsitebuilderRes = $oRVsitebuilderLicense->Create();
				//print_r($rvsitebuilderRes);
				
				if ($rvsitebuilderRes== false) {
					return false;
				}
			}
	...
	{return false;
		...
		$rvsitebuilderRes = $cpanelRes = false;
		$oRVsitebuilderLicense= $this->setRVsitebuilderModule();
		$rvsitebuilderRes= $oRVsitebuilderLicense->Suspend();
		if ($rvsitebuilderRes == false) {
			return false;
		}
	...
	{return false;
		...
		$rvsitebuilderRes = $cpanelRes = false;
		$oRVsitebuilderLicense= $this->setRVsitebuilderModule();
		$rvsitebuilderRes= $oRVsitebuilderLicense->Unsuspend();
		if ($rvsitebuilderRes == false) {
			return false;
		}
	...
	{return false;
		...
		$rvsitebuilderRes = $cpanelRes = false;
		$oRVsitebuilderLicense= $this->setRVsitebuilderModule();
		$rvsitebuilderRes= $oRVsitebuilderLicense->Terminate();
		if ($rvsitebuilderRes == false) {
			return false;
		}
	...
	public function Renewal()
	{
		...
		$rvsitebuilderRes = $cpanelRes = false;
		$oRVsitebuilderLicense= $this->setRVsitebuilderModule();
		$rvsitebuilderRes= $oRVsitebuilderLicense->Renewal();
		if ($rvsitebuilderRes == false) {
			return false;
		}
