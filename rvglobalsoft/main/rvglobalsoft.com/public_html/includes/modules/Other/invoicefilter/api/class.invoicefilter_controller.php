...
class invoicefilter_controller extends HBController {
    ...
    public function invoiceNumber ($request)
    {
        ...
        $invoicePrefix      = date('ym', $currentTime) .'-%';
        $invoicePrefixHS    = 'HS'. $invoicePrefix;
        $invoicePrefixHN    = 'HN'. $invoicePrefix;
        ...
        $sqlFormat  = "
                    ...
                    (
                        ...
                        OR i.invoice_number LIKE '%s'
                    )
                    %s
                    ";
        $sql        = sprintf($sqlFormat,$sqlSelect, $invoicePrefixHS, $invoicePrefixHN, '');
        $result     = $db->query($sql)->fetch();
        ...
        $sql        = sprintf($sqlFormat,$sqlSelect, $invoicePrefixHS, $invoicePrefixHN, $sqlLimit);
        $result     = $db->query($sql)->fetchAll();
