	...
	<div class="wbox_content" id="ccpcontent">
            <table  width="100%" cellpadding="0" cellspacing="0"  class="checker">
                ...
                <tbody id="newcard" {if $dtaus}style="display:none"{/if}>
                    <tr><td align="right">{$xlang.name}</td><td><input type="text" name="dtaus[name]" size="25" value="{if $cadetails.companyname != ''}{$cadetails.companyname}{else}{$cadetails.firstname} {$cadetails.lastname}{/if}" maxlenght="16"  class="styled"  autocomplete="off"/></td></tr>
