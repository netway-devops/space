...
<form action="" method="post">
    <div class="lighterblue" style="padding: 10px">
        ...
        <div style="padding: 10px">
            <a href="" class="btn btn-default"
               onclick="{literal}$(this).slideUp(500, function() {$('.howitworks').slideDown(1000);}); return false;{/literal}">How
                it works?</a>

            <input style="font-weight: bold;" class="btn btn-primary" type="submit" name="savecharges"
                   value="Save Changes"/>
        </div>
    </div>
                        ...
                        {foreach from=$curx item=ci}
                            <td>
                                <div class="currency" {if $ci.sign}data-sign="{$ci.sign}"{/if}>
                                    <input class="form-control text-right" type="text" value="{$g.amount[$ci.id]}"
                                           name="gateways[{$gid}][currencies][{$ci.id}]"/>
                                </div>
                            </td>
                            <td>
                                <div class="percent">
                                    <input class="form-control" type="text" value="{$g.percent[$ci.id]}"
                                           name="gateways[{$gid}][percent][{$ci.id}]"/>
                                </div>
                            </td>
                        {/foreach}
                        <td></td>
    ...
    {if count($gateways)>20}
        <div class="lighterblue" style="padding: 10px">
            <div style="padding: 10px">
                <input style="font-weight: bold;" type="submit" class="btn btn-primary" name="savecharges"
                       value="Save Changes"/>
            </div>
        </div>
    {/if}
