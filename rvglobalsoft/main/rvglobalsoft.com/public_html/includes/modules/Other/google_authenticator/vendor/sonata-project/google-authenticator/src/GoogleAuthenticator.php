...
final class GoogleAuthenticator implements GoogleAuthenticatorInterface
{
     ...
     */
    public function __construct($passCodeLength = 6, $secretLength = 10, \DateTimeInterface $now = null)
    {
        $this->passCodeLength = $passCodeLength;
        $this->secretLength = $secretLength;
        $this->pinModulo = 10 ** $passCodeLength;
        $this->now = $now ? $now : new \DateTimeImmutable();
    }
    ...
    public function getUrl($user, $hostname, $secret)
    {
        ...
        $issuer = func_get_args()[3] ? func_get_args()[3] : null;
