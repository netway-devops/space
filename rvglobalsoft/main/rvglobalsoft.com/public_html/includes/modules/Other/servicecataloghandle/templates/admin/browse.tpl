    ...
    $.getJSON('?cmd=servicecataloghandle&action=listServiceCatalog&catId='+ $catId +'&type={/literal}{$oInfo->type}{literal}', function (a) {
        ...
        $.each(oDatas, function (index, oData) {
            ...
            str         += '<tr>'
                ...
                + '<td style="border-bottom: 1px dotted #AAAAAA;"><a href="?cmd=servicecataloghandle&action=view{/literal}{if $oInfo->type == 'incidentKB'}IncidentKB{/if}{literal}&id='+ oData.id +'" target="_blank">'+ oData.title +'</a></td>'
                ...
                + '<a href="?cmd=servicecataloghandle&action=delete{/literal}{if $oInfo->type == 'incidentKB'}IncidentKB{else}ServiceCatalog{/if}{literal}&id='+ oData.id +'" onclick="return confirm(\'ยืนยันการลบ?\');"><span style="color:red; '+ eStyle +'">ลบ</span></a>&nbsp;'
...
<div class="mmdescr">
    ...
    <ul id="grab-sorter" style="width:100%">
        ...
        <li style="background-color: {if $aCat.level == '--- '} #F5F9FF; {else} #FFFFFF; {/if} line-height: 24px;">
            ...
            <tr>
                <td width="70%">
                    <a href="#{$aCat.id}" onclick="listServiceCatalog($(this), {$aCat.id}, '{$aCat.level}')" tabindex="{$aCat.id}" title="{$aCat.level}">{$aCat.level} {$aCat.name} {if $aTotal[$aCat.id]} ({$aTotal[$aCat.id]}) {/if}</a>
                </td>
                <td class="mrow1">
                    <a name="#{$aCat.id}"></a>
