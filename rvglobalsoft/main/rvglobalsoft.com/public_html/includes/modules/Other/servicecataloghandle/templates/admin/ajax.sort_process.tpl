...
<div id="formcontainer">
    <div id="formloader">
        ...
        <tr>
            ...
            <td class="conv_content form"  valign="top">
                ...
                <div class="tabb">
                    <h3>เรียงลำดับ Fulfillment Process</h3>
                    
                    <!-- Start content -->
                    
                    <p style="margin-bottom:0px;">ลากเพื่อจัดลำดับการแสดงผลก่อนหลัง</p>
                    
                    <h4>{if $aProcess|@count}จัดลำดับหมวด{/if}</h4>
                    <ul id="sortProcess" class="sortableItem">
                        {if count($aProcess)}
                        {foreach from=$aProcess item=aPro}
                        <li value="{$aPro.id}">#{$aPro.id} {$aPro.name}</li>
                        {/foreach}
                        {/if}
                    </ul>
    ...
    $(document).ready( function () {
        ...
        var catLength       = {/literal}{$aProcess|@count}{literal};
        $('#sortProcess').sortable().bind('sortupdate', function() {
            $('.spinner').show();
            var sorts   = '0';
            $('#sortProcess li').each( function (i) {
                if (i == catLength) {
                    return false;
                }
                sorts   += ','+ $(this).attr('value');
            });
            $.post('?cmd=servicecataloghandle&action=orderProcess', {
