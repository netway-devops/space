...
class kbank extends PaymentModule {
    
    // Extras >> Plugins >> Hosting Modules
    // Module name
    protected $modname = "Credit Card (Pay Now)";
    ...
    protected $configuration = array(
        ...
        'IS_NETWAY_MERCHANT' => array(
            'value' => '0',
            'type' => 'check',
            'description' => 'Check ว่า Netway เป็นผู้รับเงิน'
        )
    );
    ...
    public function drawForm() {
        ...
        $amountNetFormat = sprintf('%012s', $amountNet);
        $string = "<form action='https://rt05.kasikornbank.com/pgpayment/payment.aspx' name='paykbank'method='POST'>";
         
         //draw hidden fields with payment details
         $string.="<input type='hidden' name='invoice_id' value='{$this->invoice_id}'/>";
         $string.="<input type='hidden' name='amount' value='{$amountNet}'/>";
         $string.="<input type='hidden' name='currency_code' value='".$this->getCurrency()."'/>";

         //draw hidden field with return url
         $string.="<input type='hidden' name='URL2' value='{$url}?cmd=clientarea&action=invoice&id={$this->invoice_id}'/>";
         $string.="<input type='submit' value='Pay now!' />";
          //draw hidden fields with kBank
         $string.="<input type='hidden' name='MERCHANT2' value='{$this->configuration['MERCHANT2']['value']}'/>";
         $string.="<input type='hidden' name='TERM2' value='{$this->configuration['TERM2']['value']}'/>";
         $string.="<input type='hidden' name='RESPURL' value='{$this->callback_url}'/>";
         $string.="<input type='hidden' name='IPCUST2' value='{$this->configuration['IPCUST2']['value']}'/>";
         $string.="<input type='hidden' name='INVMERCHANT' value='{$this->invoice_id}'/>";
         $string.="<input type='hidden' name='FILLSPACE' value='{$this->configuration['FILLSPACE']['value']}'/>";
         $string.="<input type='hidden' name='AMOUNT2' value='{$amountNet}'/>";
         $string.="<input type='hidden' name='DETAIL2' value='{$this->invoice_id}'/>";
	...
	function _verifykbank($codeKBank)
	{
      ...
      //$_POST['PMGWRESP'] = "00XXXXXXXXXXXX009352XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX00000000327111122013140032000000048150XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXKBANKCARDXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX764XXXXXXXXXXXX";
	function callback() 
	{
