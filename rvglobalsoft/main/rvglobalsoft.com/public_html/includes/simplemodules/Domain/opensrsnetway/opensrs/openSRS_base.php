		...
		if ($actConfig != false){
			...
			if ($fileConfig != false){
				...
				foreach ($fileFileArray as $ffa){
					...
					if ($ffa['name'] == "osrs_debug" && isSet($ffa['value']))  $this->osrs_debug = $ffa['value'];
				}
...
//		Close the socket connection
	...
	private function read_data() {
		...
		if (isSet($this->osrs_debug)) print_r("<pre>" . htmlentities($data) . "</pre>");
	...
	private function send_data($message) {
		if (isSet($this->osrs_debug)) print_r("<pre>" . htmlentities($message) . "</pre>");
