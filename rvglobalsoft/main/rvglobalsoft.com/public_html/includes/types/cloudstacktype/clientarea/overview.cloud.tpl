        ...
        <tbody id="updater">
            {if $MyVMs}
                {foreach from=$MyVMs item=vm name=foo}
                    <tr >
                        <td >
                            {if $vm.built=='true'}{if $vm.power=='true'}
                                ...
                                {/if}
                            {else}
                                <a  class="iphone_switch_container iphone_switch_container_pending"><img src="includes/types/cloudstacktype/images/iphone_switch_container_off.png" alt="" /></a>
                            {/if}
                        </td>
                        ...
                        <td>
                            {foreach from=$vm.ip item=ipp key=nic name=ssff}
                                ...
                                {elseif $vm.source_nat[$nic]}
                                    <span class="vtip_description" style="background: none; padding: 0;" title="Source NAT, Local IP: {$ipp}">{$vm.source_nat[$nic]}*</span>
                                {else}{$ipp}
                                {/if}
