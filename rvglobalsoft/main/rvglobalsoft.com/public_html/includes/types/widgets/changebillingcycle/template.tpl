                                ...
                                <div class="row">
                                    <div class="col-12 col-md-10 col-lg-8">
                                        <select name="cnewcycle" class="form-control">
                                            {if $newcycles.h!=0}
                                                <option value="h">{$newcycles.h|price:$currency:true:true} {$lang.h}</option>{/if}
                                            {if $newcycles.d!=0}
                                                <option value="d">{$newcycles.d|price:$currency:true:true} {$lang.d}</option>{/if}
                                            {if $newcycles.w!=0}
                                                <option value="w">{$newcycles.w|price:$currency:true:true} {$lang.w}</option>{/if}
                                            {if $newcycles.m!=0}
                                                <option value="m">{$newcycles.m|price:$currency:true:true} {$lang.m}</option>{/if}
                                            {if $newcycles.q!=0}
                                                <option value="q">{$newcycles.q|price:$currency:true:true} {$lang.q}</option>{/if}
                                            {if $newcycles.s!=0}
                                                <option value="s">{$newcycles.s|price:$currency:true:true} {$lang.s}</option>{/if}
                                            {if $newcycles.a!=0}
                                                <option value="a">{$newcycles.a|price:$currency:true:true} {$lang.Annually}</option>{/if}
                                            {if $newcycles.b!=0}
                                                <option value="b">{$newcycles.b|price:$currency:true:true} {$lang.b}</option>{/if}
                                            {if $newcycles.t!=0}
                                                <option value="t">{$newcycles.t|price:$currency:true:true} {$lang.t}</option>{/if}
                                        </select>
                                    </div>
                                </div>
                            </td>
