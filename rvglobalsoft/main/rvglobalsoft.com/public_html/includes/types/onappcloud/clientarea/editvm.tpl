...
<div class="content-bar nopadding" style="position:relative">
            ...
            <tr>
                <td colspan="2">
                    <div class='input-with-slider'>
                        ...
                        <input type="text" size="4" required="required" name="CreateVM[cpus]" class="styled" value="{if $VMDetails.cpus}{$VMDetails.cpus}{else}{$CreateVM.limits.cpu}{/if}" id="virtual_machine_cpus"/>

                        <div class='slider' max='{$CreateVM.limits.cpu}' min='1' step='1' target='#virtual_machine_cpus'  total="{$CreateVM.limits.cpus}" divide="#virtual_machine_cpu_shares"></div>
                    </div>
  ...
  {if !$vcpu}
            <tr>
                <td colspan="2">
                    <div class='input-with-slider'>
                        <span class="slabel">{$lang.cpuprio}</span>
                        <input type="text" size="4" required="required" name="CreateVM[cpu_shares]" class="styled" value="{if $VMDetails.cpu_shares}{$VMDetails.cpu_shares}{else}{$CreateVM.limits.cpus}{/if}" id="virtual_machine_cpu_shares"/>
                        %
                        <div class='slider' max='{$CreateVM.limits.cpus}' min='1' step='1' target='#virtual_machine_cpu_shares'></div>
                    </div>
