...
<ul class="sub-ul">
    {if $o_sections.o_rdns}<li ><a  href="?cmd=clientarea&action=services&service={$service.id}&vpsdo=ips&vpsid={$vpsid}&widget=o_rdns" class="{if $vpsdo=='firewall'}active{/if}"><span>Reverse DNS</span></a></li>{/if}
