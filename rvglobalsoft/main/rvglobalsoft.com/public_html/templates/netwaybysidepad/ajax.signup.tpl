                            ...
                            <td class=""> 
                                <label for="field_{$k}" >
                                    {if $k=='type'}
                                        {$lang.clacctype}
                                    {elseif  $k == 'newsletter'}
                                        <span style="padding-left: 10px; line-height: 1em;">{$field.name}</span>
                                    {elseif $field.options & 1}
                                        {if $lang[$k]}{$lang[$k]}{else}{$field.name}{/if}
                                    {else}
                                        {$field.name}
                                ...
                                {elseif  $k == 'newsletter'}
                                    {foreach from=$field.default_value item=fa}
                                        <input type="checkbox" id="field_{$k}" name="{$k}[{$fa}]" {if isset($submit[$k]) && ! $submit[$k][$fa]} {else} checked="checked" {/if} value="1" style="float: left;" />
                                    {/foreach}
                                {else}
                                    {if $field.field_type=='Input'}
                                        {if  $k=='captcha'}
                     ...
                     <div id="mAddrForm" class="form-horizontal" style="width:80%; display:block; background-color:#F9F9FB; padding:3px; border:1px solid #CCCCCC; {if !$isMailingAddress} display:none; {/if}">
                        ...
                        <div class="control-group">
                            <div class="controls form-inline">
                            	<label for="mAddrProvince">{$lang.mailingProvince}:</label>
