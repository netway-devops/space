...
include($templatePath . '/cart.tpl.php');
unset($_SESSION['ADD_LOADER_CART_SSL']);
{/php}
...
<div class="wbox">
    <div class="wbox_header">
        ...
        {if $islicence}
            <div class="fright"><a href="{$ca_url}cart/licenses/" class="btn  btn-success" >Add more license</a></div>
        {/if}
        ...
        <table cellspacing="0" cellpadding="0" border="0" width="100%" class="checker table">
            ...
            {if $contents}
                {foreach from=$contents item=order key=k}
                    <tbody class="{if $k == $cartIndex} alert {/if}">
                        {if $order.product}
                            ...
                            <tr class="srow">
                                <td colspan="3">
                                    <div class="alert alert-error">
                                        <strong>License not available</strong><br />
                                        Please remove or edit this item detail to continue.
                                    </div>
                                </td>
                            </tr>
                            {/if}
                        ...
                        {if $order.product_configuration}
                            {foreach from=$order.product_configuration item=cstom2}
                                {foreach from=$cstom2 item=cstom}
                                    ...
                                    {elseif ($cstom.name == "IP Address" || $cstom.variable == 'ip') && $order.product.category_id == 6}
                                        <tr >
                                            <td valign="top" >
                                                {$cstom.fullname}{$cstom.val}
            ...
            {if $tax}
                <tr >
                    ...
                    <td >{if isset($2faProrata)}{$currency.sign}{$2faProrata} {$currency.code}{else}{$tax.subtotal|price:$currency}{/if}</td>
                </tr>
            ...
            <tr >
                ...
                <td  style="border:none">
                    ...
                    {if $tax}
                        <span class="cart_total">{if isset($2faProrata)}{$2faProrata}{else}{$tax.total|price:$currency:false}{/if}</span>
                    {elseif $credit}
...
{if $isPartnerOrderOurLicense}
    <div class="wbox">
        <div class="wbox_header">
            <strong>Your account is NOC Partner with RVGlobalSoft</strong>
        </div>
        <div class="wbox_content suggested" id="slides">
            <div class="suggested-products row text-error" align="center">
                You can issue RVSkin , RVSitebuilder licenses in more easier way at your account under NOC Licenses page.
            </div>
        </div>
    </div>
{/if}
...
{/if}
<form action="{$ca_url}cart" method="post" id="chkOutFrm">
    <input type="hidden" name="make" value="checkout" />
    <div class="wbox" {if $tax && $tax.total==0}style="display:none"{elseif $credit && $credit.total==0}style="display:none"{elseif $subtotal.total==0}style="display:none"{elseif isset($2faRenew) && $2faRenew == 1}style='display:none'{/if}>
        <div class="wbox_header">
            ...
            {if $gateways}
                <center>
                    {foreach from=$gateways item=module key=mid name=payloop}
                        {if $module == 'Credit Card (subscribe)' && $smarty.session.SSLITEM.isSSLOrder == '1'}{continue}{/if}
                        {if $mid != 121}
                            {if $mid != 122}
            ...
            <div class="wbox_header">
                ...
                <ul class="wbox_menu">
                    ...
                    <li {if $submit.cust_method=='login'}class="bgon2"{else}class="bgoff2"{/if}>
                        <input type="radio" name="cust_method" value="login" {if $submit.cust_method=='login'}checked='checked'{/if} onClick="{literal}ajax_update('index.php?cmd=login&isAjaxLogin=1&isReloadContent=1', {layer: 'ajax'}, '#updater', true);
                            $(this).parent().parent().find('li.bgon2').removeClass('bgon2').addClass('bgoff2');
                            $(this).parent().removeClass('bgoff2').addClass('bgon2');{/literal}" />
    ...
    <div class="wbox">
        ...
        <div class="wbox_content">
            ...
            <table border="0" cellpadding="0" cellspacing="6" width="100%">
                ...
                {if $tos}
                    <tr>
                        <td><input type="checkbox" value="1" name="tos"/> {$lang.tos1} <a href="{$tos}" target="_blank">{$lang.tos2}</a><a id="er" style="color:red;display:none;">&nbsp;&nbsp;&nbsp;**</a></td>
                    </tr>
                    {if $isOrderRVSiteBuilder}
                    <tr>
                        <td><input type="checkbox" value="1" name="lam"/> By clicking check out you are accepting to our <a href="https://www.rvsitebuilder.com/license-agreement" target="_blank">License Agreement</a><a id="er" style="color:red;display:none;">&nbsp;&nbsp;&nbsp;**</a></td>
                    </tr>
                    {/if}
                {/if}
            </table>
    ...
    <div class="orderbox"  id="checkout">
        <div class="orderboxpadding">
            <center>
                <input type="submit" value="{$lang.checkout}" name="submit" class="padded btn  btn-primary" onclick="{if $cartIndex >= 0} alert('License not available, please remove or edit item detail to continue.'); return false; {/if}"/>
                <input type="submit" value="{$lang.clear_cart}" name="clear" class="padded btn" onclick="removeCartItems(); return false;"/>
            </center>
        </div>
    </div>
</form>
...
var cartItems       = {/literal}{$cartItems}{literal};
...
function removeCartItems ()
{
    if(cartItems > 0) {
        var myIntv  = setInterval(function (){
            ...
            if (cartItems <= 0) {
            	document.location = '{/literal}{$system_url}{$ca_url}{literal}order/';
            }
        }, 1500);
    } else{
        document.location = '{/literal}{$system_url}{$ca_url}{literal}order/';
...
$(document).ready( function () {
    $('form').submit(function(){
        if($('input[name="tos"]').prop('checked')){
            ...
            {if $isOrderRVSiteBuilder}
            {literal}
            if(! $('input[name="lam"]').prop('checked')){
                $('#er').show();
                alert('Please check accepted license agreement');
                return false;
            }
            {/literal}
            {/if}
            ...
            if($("input[name=cust_method]:checked").val()== 'login'){
                var error = "You've not logged in to system. Please login to your account first.";
                ...
                try{
                    gao('send', 'event', 'order', 'order', 'order', 1);
                } catch(err){}
              return;
            }
            ...
            $('#er').show();
            alert('Please check accepted term of Sevice');
            return false;
        }
