...
include($templatePath . 'support/viewticket.tpl.php');
{/php}
...
<div class="wrapper-bg">
    ...
    <div class="services-content">
        ...
        <div class="row-fluid">
            <div class="span6 pull-right">
                ...
                <tr>
                    ...
                    <th>
                        {if $ticket.status == 'Closed'}
                        <a href="?cmd=supporthandle&action=deleteCustomfield&ticketId={$ticket.id}" 
                        onclick="{literal}if(confirm('Confirm delete?')){$.post($(this).attr('href'), {}, function(a) { parse_response(a); location.reload(); });} return false;{/literal}"
                        class="btn btn-danger pull-right"><span class="icon-remove icon-white"></span> Delete</a>
                        {else}
                        <a href="javascript:void(0);" 
                        onclick="$('.cf-show').hide();$('.cf-edit').show();$(this).hide();"
                        class="btn btn-primary pull-right"><span class="icon-edit icon-white"></span> Edit</a>
                        {/if}
                    </th>
                </tr>
                </thead>
                ...
                <tr class="cf-show">
                    <td>{if isset($lang.$kLang)}{$lang.$kLang}{else}{$k}{/if}</td>
                    <td>{$v}</td>
                </tr>
                <tr class="cf-edit">
                    <td>{if isset($lang.$kLang)}{$lang.$kLang}{else}{$k}{/if}</td>
                    <td><input type="{if preg_match('/password/i', $k)}password{else}text{/if}" name="cf[{$k}]" value="{$v}" size="60" /></td>
                </tr>
                {/foreach}
                <tr class="cf-edit">
                    <td></td>
                    <td><button id="customfieldUpdate" class="btn btn-success">Update</button></td>
                </tr>
                </tbody>
                </table>
            </div>
        ...
        $(document).ready( function () {
            $('.cf-edit').hide();
            $('#customfieldUpdate').click( function () {
                $.post('?cmd=supporthandle&action=updateCustomfield&ticketId={/literal}{$ticket.id}{literal}', $('input[name^=cf]').serialize(), function(a) {
                    parse_response(a); 
                    location.reload(); 
                });
                return false;
