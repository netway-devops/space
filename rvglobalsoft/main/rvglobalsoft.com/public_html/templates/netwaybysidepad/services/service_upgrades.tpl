...
{elseif $upgrades && $upgrades!=-1 && $service.slug != 'rv2factor'}
        <form action="{if $upgrades.0.id==59}{$system}index.php?cmd=order2factorhb&action=setCartUpgrade{/if}" method="post">
            <input type="hidden" value="upgrade" name="make" />
            <input type="hidden" value="upgrade" name="do" />
            <input type="hidden" value="{$service.id}" name="aid" />
      ...
      {/if}
		 	 ...
		 	 {/foreach}
                    ...
                    {if $i.catname=='RV2Factor'}
                        <input type="text" name="qty" size ="5" value="{$i.qty}" />
                    {/if}
