    ...
    <td class="period">
        <select  style="padding: 0;" name="period[{$check.sld}{$check.tld}]">
            {foreach from=$check.prices item=price}
                {if !$transfer && $price.register>=0}
                    <option value="{$price.period}">
                        {$price.period} {$lang.years} @ {$price.register|price:$currency}
                    </option>
                {/if}
                {if $transfer && $price.transfer>=0}
                    <option value="{$price.period}">
                        {$price.period} {$lang.years} @ {$price.transfer|price:$currency}
                    </option>
                {/if}
