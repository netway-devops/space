...
include(dirname($templatePath) . '/orderpages/cart4.tpl.php');
{/php}
...
<div class="default-cart"></div>
 ...
 <!-- START RV ADD FORM-->
{if $contents[0].name=='2-factor Authentication' && $logged=="1"}
     <div class="orderbox"  id="checkout">
        <div class="orderboxpadding">
            <center>
               <input type="button" value="{$lang.checkout}" style="font-size:12px;font-weight:bold"  class="padded btn  btn-primary" onclick="jQuery('#cart4-submit').submit();" />  
               <input type="button" value="{$lang.clear_cart}" name="clear" style="font-size:12px;" class="padded btn" onclick="window.location='{$ca_url}cart/';" />
            </center>
        </div>
    </div>  
{/if}   
...
<!-- Start Sidemenu -->
<div id="sidemenu">
    <div class="client-sidemenu">
        <div class="client-padd"><img src="{$template_dir}images/paypal.jpg" alt="" width="382" height="51" /></div>
        <!--<div class="client-promo"><a href="{$system_url}knowledgebase/article/19/what-is-promotional-code-/" target="_blank"><img src="{$template_dir}images/use-promotion.gif" alt="" width="173" height="22" /></a></div>
        <div class="client-what"><a href="{$system_url}knowledgebase/article/19/what-is-promotional-code-/" target="_blank" class="what">What is promotional code ?</a></div>-->
    </div>
  <div class="wbox">
    <div class="wbox_header wbox_header_summary">
...
{/if}
    ...
    <div class="wbox_content wbox_content_summary wbox_content_pad">
<table cellspacing="0" cellpadding="0" border="0" align="center" width="100%" class="checker table table-summary" style="margin:0;">
  <tbody>
    <tr>
      <th width="45%" align="left" class="bdrl">{$lang.Description}</th>
    
      <th width="30%"> {$lang.setupfee}</th>
      <th width="25%">{$lang.price}</th>
    </tr>
    {if $product}
    
        {if $contents[0].name=='2-factor Authentication'}
               
               <tr class="srow">
                    <td valign="top" class="bdrl">
                        <strong>{$lang.Rv_Vip_Order_Desc}</strong><br/>
                    </td>
                    <td align="center">
                        {0.00|price:$currency}
                   </td>
        ...
        {else}
            ...
            <tr class="srow">
              <td valign="top" class="bdrl">
                ...
                {if $contents[2].domain}({$contents[2].domain}){/if}
                {if $SSLCommonName}({$SSLCommonName}){/if}
                <br/>
              </td>
    ...
    {if $cart_contents[1]}
            ...
            {if $cstom.total>0}
                 ...
                 {if $cstom.name=="Symantec VIP Account"}
                 {else}
                     <tr >
                           <td valign="top" class="blighter fs11 bdrl" style="padding-left:15px">{$cstom.fullname}  {if $cstom.qty>1}x {$cstom.qty}{/if}<br/></td>
  ...
  <tr >
    <td class="bdrl">{$lang.addon} <strong>{$addon.name}</strong></td>
  ...
  {/if} {if $product}
  <tr >
    <td class="bdrl"><a style="color:#ff7200;" href="{$ca_url}cart&amp;step=3"  class="fs11">[{$lang.edit_config}]</a> {if !$contents[2] && $contents[0].domainoptions}<a style="color:#ff7200;" href="{$ca_url}cart&amp;step=1"  class="fs11">[{$lang.add_domain}]</a>{/if}</td>
    <td colspan="2"></td>
  </tr>
  {/if}
  {if $contents[2] }
      {foreach from=$contents[2] item=domenka key=kk}{if $domenka.action!='own' && $domenka.action!='hostname'}
          ...
          <tr >
            <td class="bdrl"><strong>{if $domenka.action=='register'}{$lang.domainregister}{elseif $domenka.action=='transfer'}{$lang.domaintransfer}{elseif $domenka.action=='renew'}{$lang.domainrenewal}{/if}</strong> - {$domenka.name} - {$domenka.period} {$lang.years}
                <br/>
           ...
           {if $domenka.forms}
                                ...
                                {if $cstom.total>0}
                                <tr >
                                      <td valign="top" class="blighter fs11 bdrl" style="padding-left:15px">{$cstom.fullname}  {if $cstom.qty>1}x {$cstom.qty}{/if}<br/>
                                      </td>
                                          <td align="center" class="blighter fs11">{if $cstom.setup!=0}{$cstom.setup|price:$currency}{else}-{/if}</td>
                                      <td align="center" class="blighter fs11"><strong>{if $cstom.price==0}{$lang.Free}{elseif $cstom.prorata_amount}{$cstom.prorata_amount|price:$currency}{else}{$cstom.price|price:$currency}{/if}</strong></td>
                                    </tr>
                        ...
                        {/if}
          <tr >
            <td class="blighter fs11 bdrl" style="padding-left:15px"><a style="color:#ff7200;" href="{$ca_url}cart&amp;step=2" class="fs11">[{$lang.config_extras}]</a> <a style="color: rgb(204, 0, 0);"  href="{$ca_url}cart&amp;step=4&amp;removedomain=true&key={$kk}"  class="fs11">[{$lang.remove}]</a></td>
            <td colspan="2"></td>
          </tr>
  ...
  {if $contents[4]}{foreach from=$contents[4] item=subprod}
    <tr class="srow">
      <td valign="top" class="bdrl">{$subprod.category_name} - <strong>{$subprod.name}</strong>
      </td>
  ...
  {if $tax}
  <tr >
    <td align="right" colspan="2" class="bdrl">{$lang.subtotal}</td>
    <td align="center" >{$tax.subtotal|price:$currency}</td>
  </tr>
  
  {if $subtotal.coupon}  
   <tr >
    <td align="right" colspan="2" class="bdrl">{$lang.discount}</td>
  ...
  <tr >
    <td align="right" colspan="2" class="bdrl">{$tax.tax1name} @ {$tax.tax1}%  </td>
  ...
  <tr >
    <td align="right" colspan="2" class="bdrl">{$tax.tax2name} @ {$tax.tax2}%  </td>
    <td align="center">{$tax.taxed2|price:$currency}</td>
  </tr>
  {/if}
  
  {if $tax.credit!=0}
  <tr style="{if $isRenewDomain}display:none;{/if}">
    <td align="right" colspan="2" class="bdrl"><strong>{$lang.credit}</strong> </td>
  ...
  <tr >
    <td align="right" colspan="2" class="bdrl">{$lang.total_recurring}</td>
  ...
  {/if}
        ...
        <div id="promocode" style="display:none;">
            ...
            {$lang.code}: <input type="text" class="styled box" name="promocode"/> <input type="submit" value="GO" style="font-weight:bold" class="padded btn go"/></form>
        </div>
          {/if}
          </td>
    <td align="right">{$lang.total_today}</td>
    <td align="center">
    <span style="vertical-align: top; font-size: 18px;">{$currency.sign}</span>
    {if $tax && $isRenewDomain}
            <span class="cart_total">{$tax.total+$tax.credit|price:$currency:false}</span>
        {elseif $tax}
...
<form id="cart4-submit" name="cart4-submit" action="" method="post" onsubmit='if($("#c_notes").val()=="{$lang.c_tarea}")$("#c_notes").val("");$("#checkout").addLoader();'>
  <input type="hidden" name="make" value="step4" />
  
 <div style="float:left; width:58%;">
 <!--
 <div class="wbox"
 //{if $tax && $tax.total==0 && !$isRenewDomain}style="display:none"{elseif $credit && $credit.total==0}style="display:none"{elseif $subtotal.total==0}style="display:none"{/if}>
  ...
  <center>
  {foreach from=$gateways2 item=module key=mid name=payloop}
      <label class="btn"><input onclick="$('#gatewayform').show();ajax_update('?cmd=cart&action=getgatewayhtml&gateway_id='+$(this).val(), '', '#gatewayform',true)" type="radio" name="gateway" value="{$mid}" {if $submit && $submit.gateway==$mid||$mid==$paygateid}checked="checked"{elseif $smarty.foreach.payloop.first}checked="checked"{/if} class="pull-left" /> &nbsp; {$module}</label>
  {/foreach}
  </center>  
...
-->
  ...
  <div class="wbox">
    ...
    <ul class="wbox_menu">
        ...
        <label><input type="radio" name="cust_method" value="newone" {if !isset($submit) || $submit.cust_method=='newone'}checked='checked'{/if}  onclick="{literal}ajax_update('index.php?cmd=signup',{layer:'ajax'},'#updater',true);$(this).parent().parent().find('li.bgon2').removeClass('bgon2').addClass('bgoff2');$(this).parent().removeClass('bgoff2').addClass('bgon2');{/literal}" />
        {$lang.newclient}</label></li>
        <li {if $submit.cust_method=='login'}class="bgon2"{else}class="bgoff2"{/if}>
         <label><input type="radio" name="cust_method" value="login" {if $submit.cust_method=='login'}checked='checked'{/if} onClick="{literal}ajax_update('index.php?cmd=login',{layer:'ajax'},'#updater',true);$(this).parent().parent().find('li.bgon2').removeClass('bgon2').addClass('bgoff2');$(this).parent().removeClass('bgoff2').addClass('bgon2');{/literal}" />
        {$lang.alreadyclient}</label></li>
    </ul>
    </div>
    <div class="wbox_content">
