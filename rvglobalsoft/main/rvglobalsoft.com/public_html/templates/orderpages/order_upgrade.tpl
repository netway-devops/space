...
include(dirname($templatePath) . '/orderpages/order_upgrade.tpl.php');
{/php}
{if $contents.product.product_id == '58' && $contents.product.new_product_id == '59'} 
    <div class="notifications alert alert-error" style="display:block">
        <button type="button" class="close" data-dismiss="alert">&times;</button>
        <span>You can define the quantity for RV2Factor Account in this upgrade to paid account only. Please make sure the quantity you need, and continue the upgrade to paid account.</span>
    </div>
    <br />
{/if}
            ...
            <tbody>
                ...
                {if $contents.product && $contents.product.product_id != $contents.product.new_product_id}
                    <tr class="srow">
                        ...
                        <td align="center"><strong>{if $contents.product.new_product_id == 59}&nbsp;- &nbsp; (Prorata {$contents.product.nextdue}){else}{$contents.product.charge|price:$currency}{/if}</strong></td>
                    </tr>
                {/if}
...
<form action="{if $contents.product.new_product_id == 59 && $contents.product.product_id == 58}{$system}index.php?cmd=order2factorhb&action=addInvoice{/if}" method="post" onsubmit='$("#checkout").addLoader();'>
    ...
    <div class="orderbox" id="checkout">
        <div class="orderboxpadding">
            <center>
                ...
                {$lang.or}
                <a href="?cmd=clientarea&action=services&service={$contents.product.account_id}"
