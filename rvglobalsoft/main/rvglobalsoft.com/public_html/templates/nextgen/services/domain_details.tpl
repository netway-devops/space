{if $bulkdetails}
    <ul class="breadcrumb">
        ...
        <li><a href="{$ca_url}clientarea/domains/"><strong>{$lang.domains}</strong></a> <span class="divider">/</span>
        </li>

        {if $widget}
            <li><a href="{$ca_url}clientarea/services/domains/{$service.id}/"><strong>{$details.name}</strong></a> <span
...
{elseif $details}
        ...
        <div class="wbox" style="display:none" id="huregistrar">
            ...
            <div class="wbox_content">
                ...
                <div style="padding:5px;">
                    <form style="padding:10px 0 0;" action="" method="POST" enctype="multipart/form-data" id="hu_upform"
                          class="form-inline">
                        <input type="file" name="signedform"> <input class="btn btn-primary" type="submit"
                                                                     value="{$lang.hu_sendform}"
    ...
    <div class="row flex-container bordered-section">
        ...
        <div class="span3 left-content flex-box-1">
            ...
            <ul class="leftmenu">
                <li {if !$widget}class="current"{/if}><a
                            href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/"><span
                                class="home">{$lang.domdetails}</span></a></li>
                {foreach from=$widgets item=widg name=cst}{if $widg.name!='reglock' && $widg.name!='nameservers'  && $widg.name!='autorenew' } {if $widg.name=='idprotection' && $details.offerprotection && !$details.offerprotection.purchased}{continue}{/if}
                    <li {if $widget.name==$widg.name}class="current"{/if}><a
                                href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/&widget={$widg.name}{if $widg.id}&wid={$widg.id}{/if}#{$widg.name}">
        ...
        <div class="span9 right-content flex-box-1">
            ...
            <div class="brcrm">
                <ul class="breadcrumb left">
                    <li><a href="{$ca_url}clientarea/"><strong>{$lang.clientarea}</strong></a> <span
                                class="divider">/</span></li>
                    <li><a href="{$ca_url}clientarea/domains/"><strong>{$lang.domains}</strong></a> <span
                                class="divider">/</span></li>

                    {if $widget}
                        <li>
                            <a href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/"><strong>{$details.name}</strong></a>
                            <span class="divider">/</span></li>
                        <li class="active">{if $lang[$widget.name]}{$lang[$widget.name]}{elseif $widget.fullname}{$widget.fullname}{else}{$widget.name}{/if}</li>{else}
                        <li class="active">{$details.name}</li>{/if}
                </ul>
                <a href="{$ca_url}clientarea/domains/" class="right btn btn-mini"><i
                            class="icon-chevron-left"></i> {$lang.back}</a>
                <div class="clear"></div>
            </div>
                ...
                <div class="p19 serviceinfo">
                    ...
                    <div class="row">
                        ...
                        {if $details.status == 'Active' || $details.status == 'Expired'}
                            ...
                            {if !$details.expires || $details.expires == '0000-00-00'}{$lang.none}{else}
                                <h4 class="left">{$details.expires|dateformat:$date_format}</h4> {if $details.daytoexpire >= 0}
                                <small class="fs11">
                                    ({$details.daytoexpire} {if $domain.daytoexpire==1}{$lang.day}{else}{$lang.days}{/if} {$lang.toexpire}
                                    )
                                </small>
                            {/if}{/if}
                            <span class="right"> {if $allowrenew}<img src="{$template_dir}icons/shopping-basket.png"
                        ...
                        {if $details.status == 'Active'}
                            <div class="span4 mb15">
                                <h3>{$lang.reglock}: <span class="vtip_description" title="{$lang.reglock_desc}"></span>
                                </h3>
                                {if $details.reglock=='1'}<span class="label label-Active">{$lang.On}</span>{else}<span
                                        class="label label-Expired">{$lang.Off}</span>{/if}
                                <span class="right"> {foreach from=$widgets item=widg name=cst}{if $widg.name=='reglock'}
                                    <img src="{$install_url}{$widg.config.smallimg}" alt="{$widg.name}" />
                            ...
                            {if !$details.not_renew}
                                <div class="span4 mb15">
                                    <h3>{$lang.autorenew}: <span class="vtip_description"
                                                                 title="{$lang.autorenew_desc}"></span></h3>
                                    {if $details.autorenew=='1'}<span class="label label-Active">{$lang.On}</span>{else}
                                        <span class="label label-Expired">{$lang.Off}</span>{/if}
                                    <span class="right">
                                        {foreach from=$widgets item=widg name=cst}
                                        {if $widg.name=='autorenew'}<img src="{$install_url}{$widg.config.smallimg}"
                                                                         alt="{$widg.name}" />
                            ...
                            {if $details.offerprotection}
                                <div class="span4 mb15">
                                    <h3>{$lang.privacyprotection}: <span class="vtip_description"
                                                                         title="{$lang.privacyprotection_desc}"></span>
                                    </h3>
                                    {if $details.offerprotection.enabled}<span
                                            class="label label-Active">{$lang.On}</span>{else}<span
                                            class="label label-Expired">{$lang.Off}</span>{/if}

                                    {if $details.offerprotection.purchased}
                                    <span class="right"> {foreach from=$widgets item=widg name=cst}{if $widg.name=='idprotection'}
                                            <img src="{$install_url}{$widg.config.smallimg}" alt="{$widg.name}" /> <a
                                    ...
                                    {else}
                                    <span class="right"> <img src="{$template_dir}icons/shopping-basket.png"
                                                              alt="{$lang.idprotection}"/> <a
                                                href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/&make=domainaddons">{$lang.addprivacy}</a></span>
                                    {/if}&nbsp;
                                    <span class="clear"></span>
                                </div>
                            {/if}
                    ...
                    <table cellspacing="0" cellpadding="0" class="table table-striped table-bordered" width="100%">
                        ...
                        <tr>
                            <td colspan="2">
                                {foreach from=$widgets item=widg name=cst}{if $widg.name=='nameservers'}<img
