<div class="bordered-section article">
    ...
    {if $campaigns }
        <div class="p19"><h3 class="mt-10 mb-15">{$lang.affcampaigns}</h3></div>
        <table width="100%" border=0 class="table table-striped fullscreen" cellspacing="0">
            {foreach from=$campaigns item=campaign}
                <tr>
                    <td>
                        <h4>{$campaign.name}</h4>
                        <a href="{$system_url}?affid={$affiliate.id}&campaign={$campaign.campid}">{$system_url}?affid={$affiliate.id}&campaign={$campaign.campid}</a>
        ...
        <div id="landinglink" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="refModal" aria-hidden="true">
            ...
            <div class="modal-body">
                <label class="control-group">
                    ...
                    <input type="text" name="landingurl" placeholder="{$landingpage}" data-domain="{$validationhost|escape}" class="span5" {if !$affiliate.options & 1}id="lpi"{/if}>
                </label>
            </div>
