                ...
                <div class="col-sm-12">
                    <div class="text-left">
                        <form action="" method="post" class="pull-left order_methods" style="">
                            ...
                            <a class="submiter menuitm menu-auto" name="markcancelled" {if $enablequeue}queue='push'{/if} href="#" ><span >{$lang.Cancel}</span></a>
                            <a class="submiter menuitm menu-auto" name="markfraud" {if $enablequeue}queue='push'{/if} href="#"><span >{$lang.setasfraud}</span></a>
            ...
            <script>
                ...
                function confirm1() {
                    delete_cb = function (hard) {
                        ajax_update('?cmd=orders&delete=delete&' + $.param($("#form_orders input[class=check-order]:checked")) + hard, {stack: 'push'});
                    };
                ...
                function update_order(button, params) {
                    ...
                    var form = $('#form_orders').serializeArray(),
                    ...
                    var form_clientorders = $('.order_methods').serializeArray();
                    form_clientorders.push({name:'page', value:page});

                    $.post('?cmd=orders&'+button.attr('name'), form, function (data) {
                        parse_response(data);
                        ajax_update('', form_clientorders, button.parents('.slide'));
                    });
                ...
                $('#per_page').on('change', function () {
                    const form_clientorders = $('.order_methods').serializeArray();
                    form_clientorders.push({name:'orders_per_page', value:$(this).val()});
                    $('.slide:visible').addLoader();
                    ajax_update('',form_clientorders,$('.submiter').parents('.slide'));
                });
                {/literal}
            </script>
        {/if}
    ...
    {if $orders}
        {foreach from=$orders item=order}
            <tr>
                ...
                <td>{$order|@profilelink:true:true}

                  <br />
                  {if isset($aOrderDetail[$order.id]) && $aOrderDetail[$order.id]|@count}
                  {foreach from=$aOrderDetail[$order.id] item="arr"}
                  {$arr.description}<br />
                  {/foreach}
                  {/if}
