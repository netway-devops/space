...
<div class="form conv_content" style="background:#fff; padding: 30px 30px; ">
    <form method="post" action="" id="sform1"><h3>{$spec.name}</h3>

        <p>สามารถดู preview บน social ได้โดยไปที่
            <a href="https://developers.facebook.com/tools/debug/" target="_blank">
                https://developers.facebook.com/tools/debug
          </a>แล้วนำ URL ของ Order page นี้ไปทดสอบ</p>
        ...
        <div style="">
            ...
            {foreach from=$opconfig item=c key=k}
                ...
                <label for="{$k}" class="nodescr" style="margin-right: 15px; padding-top: 21px !important; ">{$c.name} </label>
                {if $c.type=='input'}
                    {hbinput type="text" class="w250" name="opconfig[`$k`]" value=$c.value style="margin-bottom:5px; width:500px;height: 23px;margin-top:15px; "}
                {*}<input type="text" class="w250" name="opconfig[{$k}]" value="{$c.value|htmlspecialchars}" style="margin-bottom:5px; width: 425px; "/>{*}
                ...
                {elseif $c.type=='textarea'}
                    {hbwysiwyg class="w250" name="opconfig[`$k`]" style="margin-bottom:5px; margin-top:15px;width: 500px; height: 120px; " value=$c.value blockwysiwyg=true}
