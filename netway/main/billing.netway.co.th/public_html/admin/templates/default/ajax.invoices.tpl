...
{if $action=='default'}
                ...
                <tbody>
                    <tr>
                        ...
                        {if $currentlist!='recurring'}
                            ...
                            {else}
                                <th><a href="?cmd=invoices&list={$currentlist}&orderby=id|ASC&listExt={$listExt}" class="sortorder">{$lang.invoicehash}</a></th>
                            {/if}
                            <th>ใบกำกับภาษี</th>
    ...
    {if $ajax}
        ...
        {if isset($reassignSorterrecords)}
...
{elseif $action=='clientinvoices'}
    ...
    $('#invoicefilter').blur(function(){
        var isFirstLoad     = $(this).attr('data-firstload');
        if (isFirstLoad == '0') {
            $(this).attr('data-firstload', '1');
            return false;
        }
        if($(this).val()!=''){
            $('#ajaxLoaderInvoice').addLoader();
            $.post('?cmd=invoicehandle&action=ajaxFilterInvoiceByClientId',
                {
            ...
            , function (data) {
                $('#preloader').remove();
                $('.sub_nav').hide();
                $('table.listinvoice').hide();
                $('center.blu').hide();
                $("#ajaxFilterInvoice").html(data.data.toString());
                $("#ajaxFilterInvoice").show();
            });

        }else{
            $("#ajaxFilterInvoice").fadeOut();
            $('.sub_nav').show();
            $('table.listinvoice').show();
            $('center.blu').show();
        }
        return false;
     });
    ...
    <div class="blu clearfix" >
        ...
        <span id="ajaxLoaderInvoice" style="float: left">
            <input onkeypress="return event.keyCode != 13;" name="invoicefilter" id="invoicefilter" type="text"
                data-firstload="0" placeholder="Fillter:Invoice ID" size="30" />
    ...
    {if $invoices}
        ...
        {if $listExt == 'clientInvoice' && $status == 'Unpaid'}
            <div style="padding: 15px;"><a href="?cmd=invoicehandle&action=listUnpaid&clientId={$client_id}" class="menuitm" target="_blank"><span class="graphst"><b>Print view</b></span></a></div>
        {/if}
        ...
        <table cellspacing="0" cellpadding="3" border="0" width="100%" class="table glike hover listinvoice">
            <tbody>
                <tr>
                    <th width="20"><input type="checkbox" id="checkall"></th>
                    {if $currentlist!='Recurring'}
                ...
                <tbody id="table-show-invoice" rowspan="2" nowrap  >
                    ...
                    {assign var="invoiceId" value=$invoice.id}
                        ...
                        {else}
                            <td rowspan="2" nowrap ><a href="?cmd=invoices&action=edit&id={$invoice.id}&list={$currentlist}" class="tload2"  rel="{$invoice.id}">{$invoice.recurring_id}</a></td>
                    ...
                    <tr>
                        <td colspan="9">{$aInvoiceServices[$invoiceId]|strip_tags:true}

                            <span id='domain-idn-{$invoice.id}'></span>
                            <script>
                                 content = `{$aInvoiceServices[$invoiceId]|strip_tags:true}`;
                                 id = 'domain-idn-{$invoice.id}';
                                 {literal}
                                 if (content.match(/Domain Registration/gi) && content.match(/xn\-\-/gi)) {

                                     content = content.replace(/[\s\S]*Domain Registration\: /gim, "");
                                     domainName = content.replace(/Period\:[\s\S]*/gim, "");
                                     ...
                                     if (domainName != '') {
                                        ...
                                        $('#' + id).html(' (domain idn: ' + punycode.decode(sld.substring(4)) + '.' + tld + ')');
                                    }
        ...
        {if $totalpages}
                ...
                <div class="col-sm-12">
                    <div class="text-left">
                        <form action="" method="post" class="pull-left invoice_methods" style="">
                            {$lang.withselected}
                            <a class="submiter menuitm menu-auto" name="markpaid" {if $enablequeue}queue='push'{/if}  href="#" ><span >{$lang.markaspaid}</span></a>
                            <a class="submiter menuitm menu-auto" name="markunpaid"  href="#" ><span >{$lang.markasunpaid}</span></a>
                    ...
                    <div style="display:inline-block">
                        <center class="blu paginercontainer" >
                            ...
                            {section name=foo loop=$totalpages}
                                <a href='?cmd=invoices&action=clientinvoices&id={$client_id}&page={$smarty.section.foo.iteration-1}&currentlist={$currentlist}&listExt={$listExt}&status={$status}' class="npaginer
            ...
            <script>
                ...
                function update_invoice(button, params) {
                    ...
                    var form = $('#form_invoices').serializeArray(),
                    ...
                    var form_clientinvoices = $('.invoice_methods').serializeArray();
                    form_clientinvoices.push({name:'page', value:page});

                    $.post('?cmd=invoices&'+button.attr('name'), form, function (data) {
                        parse_response(data);
                        ajax_update('', form_clientinvoices, button.parents('.slide'));
                    });
                ...
                var check_invoice = $('.check');
                $('#checkall').on('change', function () {
                    if ($(this).attr('checked')){
                        check_invoice.attr('checked', true);
                        check_invoice.closest('tr').addClass('checkedRow');
                    }else{
                        check_invoice.attr('checked', false);
                        check_invoice.closest('tr').removeClass('checkedRow');
                    }
                });
                check_invoice.on('change', function () {
                    if ($(this).attr('checked')){
                ...
                $('#per_page').on('change', function () {
                    const form_clientinvoices = $('.invoice_methods').serializeArray();
                    form_clientinvoices.push({name:'invoices_per_page', value:$(this).val()});
                    $('.slide:visible').addLoader();
                    ajax_update('',form_clientinvoices,$('.submiter').parents('.slide'));
                });
        ...
        <div id="ajaxFilterInvoice" style="display: none"></div>
