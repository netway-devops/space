...
{elseif $action=='getqueue'}
    ...
    <div class="service-task-list-actions">
        ...
        <div id="related-info" class="service-task-list" style="margin-top:17px">
            <strong>Related Information</strong>
            {include file='ajax.accountrelations.info.tpl' place='accounts' item_id=$account_id item_type='Hosting' client_id=$client.id items=$relations currency=$service_currency}
        </div>
...
{elseif $action=='getacctaddons'}
    ...
    <table border="0" cellpadding="2" cellspacing="0" width="100%" class="table glike" id="addontbl"
        ...
        <tr>
            ...
            <th>{$lang.Billing}</th>
            <th>{$lang.Status}
                <span class="ui-tooltips-i" title="ถ้าเปลี่ยน status เป็น Active ระบบจะทำการบันทึก invoice item ที่เกี่ยวข้องเป็น is_shipped (ส่งของแล้ว) ด้วย" style="border-bottom:1px dotted red; cursor:pointer;"><i class="fa fa-info-circle"></i></span>
            </th>
            <th></th>
...
{* custom code มีการเพิ่ม filter account  *}
    ...
    {if $accounts}
        ...
        <table cellspacing="0" cellpadding="3" border="0" width="100%" class="table glike hover">
            ...
            <tbody>
                {foreach from=$accounts item=account key=k }
                    <tr>
                        ...
                        <td>{$account.domain}
                            {if ! $account.domain}
                                <a href="?cmd=accounts&action=edit&id={$account.id}&define=domainname">Edit</a>
                            {/if}
                        </td>
                        <td>
                            {$aAccounts[$k].productCategory}
                        </td>
                        <td>{$account.name}

                          {if isset($aAccounts[$k].cPanel) && $aAccounts[$k].cPanel == '1'}
                          <div style="cursor: pointer;" class="right inlineTags">
                              <form id="login_form{$account.id}" name="login_form{$account.id}" action="https://{$account.domain}:2083/login/" method="post" target="_blank">
                                    <input name="user" id="user" value="{$aAccounts[$k].cPanelLogin.user}" type="hidden">
                                    <input name="pass" id="pass" value="{$aAccounts[$k].cPanelLogin.password}" type="hidden">
                                    <input type="hidden" name="goto_uri" value="" />
                              </form>
                              <a onclick="$('#login_form{$account.id}').submit();" >&nbsp;cPanel</a>
                          </div>
                            {if preg_match("/Reseller/i",$account.name)}
                                <div style="cursor: pointer;" class="right inlineTags">
                                  <form id="login_form_whm{$account.id}" name="login_form{$account.id}" action="https://{$account.domain}:2087/login/" method="post" target="_blank">
                                        <input name="user" id="user" value="{$aAccounts[$k].cPanelLogin.user}" type="hidden">
                                        <input name="pass" id="pass" value="{$aAccounts[$k].cPanelLogin.password}" type="hidden">
                                        <input type="hidden" name="goto_uri" value="" />
                                  </form>
                                  <a onclick="$('#login_form_whm{$account.id}').submit();">&nbsp;WHM</a>
                              </div>
                            {/if}
                          {/if}
                          
                          {if isset($aAccounts[$k].tags.netway) && $aAccounts[$k].tags.netway == '1'}<div class="right inlineTags"><span>Internal netway</span>&nbsp;</div>{/if}
                          {if isset($aAccounts[$k].tags.hypervisor) && $aAccounts[$k].tags.hypervisor == '1'}<div class="right inlineTags"><span>Hypervisor</span>&nbsp;</div>{/if}
                          {if isset($aAccounts[$k].tags.Hybrid) && $aAccounts[$k].tags.Hybrid}<div class="right inlineTags"><span>Hybrid</span>&nbsp;</div>{/if}
                        ...
                        <td>{if $account.next_due|dateformat:$date_format != ''}{if isset($aExpAccounts[$k].expire)  && $aExpAccounts[$k].expire != ''}<span style="color:{$aExpAccounts[$k].color};">{$aExpAccounts[$k].expire}</span>{elseif isset($aExpAccounts[$k].error)}{$aExpAccounts[$k].error}{else}-{/if}{/if}</td>
        ...
        {securitytoken}
            <div id="confirm_ord_delete" hidden bootbox data-title="{$lang.accdelheading}"
                ...
                <form action="?cmd=accounts&make=fulldelete" method="post">
                    ...
                    <span>{$lang.delete_account_opt2}</span>
                    <br/>
                    {securitytoken}
                </form>
        ...
        <script>
            ...
            function confirmsubmit2() {
                var add = [{
                    name: 'harddelete',
                    value:$('.bootbox-body input[name=harddelete]:checked').val()
                }];
            ...
            function update_account(button, params) {
                ...
                var form = $('#form_accounts').serializeArray(),
                ...
                var form_clientaccounts = $('.account_methods').serializeArray();
                form_clientaccounts.push({name:'page', value:page});

                $.post('?cmd=accounts&'+button.attr('name'), form, function (data) {
                    parse_response(data);
                    ajax_update('', form_clientaccounts, button.parents('.slide'));
                });
    ...
    {if $accounts}
        {foreach from=$accounts item=account key=k }
            <tr>
                ...
                {/if}
                <td>{$lang[$account.billingcycle]}</td>
                <td>{if $account.expiry_date == '0000-00-00'}-{else}{$account.expiry_date|date_format:'%d %b %Y'}{/if}</td>
                <td><span class="{$account.status}">{$lang[$account.status]}</span></td>
