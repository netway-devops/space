...
{elseif $action=='getqueue'}
    ...
    <!-- Start Detail URL Forwarding -->
    <div class="mmfeatured-inner">
        {if $rvdomainforwardingdetails}
            {include file=$rvdomainforwardingdetails}
        {/if}
    </div>
    <!-- End Detail URL Forwarding -->
...
{elseif $action=='clientdomains'}
    ...
    <script>
        {literal}
             $('#domianfilter').blur(function(){
                    ...
                    if($(this).val()!=''){
                        $('#ajaxLoaderDomain').addLoader();
                        $.post('?cmd=domainhandle&action=ajaxFilterDomainByClientId',
                            {
                        ...
                        , function (data) {
                            $('#preloader').remove();
                            $('.sub_nav').hide();
                            $('table.listdomain').hide();
                            $('center.blu').hide();
                            $("#ajaxFilterDomain").html(data.data.toString());
                            $("#ajaxFilterDomain").show();
                        });

                    }else{
                        $("#ajaxFilterDomain").fadeOut();
                        $('.sub_nav').show();
                        $('table.listdomain').show();
                        $('center.blu').show();
                    }
                    return false;
                 });
    ...
    <span id="ajaxLoaderDomain" style="float: left">
        <input onkeypress="return event.keyCode != 13;" name="domianfilter" id="domianfilter" type="text"
            data-firstload="0" placeholder="Fillter:Domain ID Or Domain Name" size="50" />
            ...
            <div class="col-sm-12">
                <div class="text-left">
                    <form action="" method="post" class="pull-left domain_methods" style="">
                        ...
                        <a class="menuitm menu-auto submiter" {if $enablequeue}queue='push'{/if} name="sync_quick"  href="#"><span>{$lang.Synchronize}</span></a>
                        <a class="menuitm menu-auto" {if $enablequeue}queue='push'{/if} onclick="return confirm_delete_selected()" href="#"><span style="color:#FF0000">{$lang.Delete}</span></a>
                ...
                <script>
                    ...
                    function confirm_delete2() {
                        ...
                        if ($('.bootbox-body input.cc_hard').is(':checked'))
                            add = '&harddelete=true';
                        if(_delete_multi){
                            ajax_update('?cmd=domains&delete=1&' + $.param($("#form_domains input[class=check]:checked")) + add, {stack: 'push'});
                        }else{
                    ...
                    function update_domain(button, params) {
                        ...
                        var form = $('#form_domains').serializeArray(),
                        ...
                        var form_clientdomains = $('.domain_methods').serializeArray();
                        form_clientdomains.push({name:'page', value:page});

                        $.post('?cmd=domains&'+button.attr('name'), form, function (data) {
                            parse_response(data);
                            ajax_update('', form_clientdomains, button.parents('.slide'));
                        });
                    ...
                    $('#per_page').on('change', function () {
                        const form_clientdomains = $('.domain_methods').serializeArray();
                        form_clientdomains.push({name:'domains_per_page', value:$(this).val()});
                        $('.slide:visible').addLoader();
                        ajax_update('',form_clientdomains,$('.submiter').parents('.slide'));
                    });
                    {/literal}
                </script>
            </div>
