...
{elseif $upgrades && $upgrades!=-1}
        <form id="upgradeService" action="" method="post">
      ...
      {/if}
		  ...
		  {/foreach}
            ...
            $(document).ready( function () {
                ...
                $('#upgradeService').submit( function () {

                    var newProductId    = $('select[name="upgrades"]').val();
                    var billingCycle    = $('select[name="cycle['+newProductId+']"]').val();
                    
                    $.post('?cmd=upgradehandle&action=upgradeService', {
                        accountId       : {/literal}{$serviceId}{literal},
                        ...
                        if (data.indexOf("<!-- {") == 0) {
                            ...
                            if (codes.DATA.length > 0) {
                                var oInvoice = $.parseJSON(codes.DATA[0]);
                                document.location = '{/literal}{$ca_url}{literal}clientarea/invoice/'+oInvoice.invoice_id+'/';
                            }
                        }
                        
                    });
                    
                    parse_response('<!-- {"ERROR":["ไม่สามารถ upgrade service ได้กรุณาลองอีกครั้ง"],"INFO":[],"STACK":0} -->');
