{if $invoices}
    ...
    {/if}
         <tr {if $smarty.foreach.foo.index%2 == 0}class="even"{/if} valign="top">
              ...
              <td class="cell-border"><a href="{$ca_url}clientarea/invoice/{$invoice.id}/" target="_blank">{if $proforma && ($invoice.status=='Paid' || $invoice.status=='Refunded') && $invoice.paid_id!=''}{$invoice.paid_id}{else}{$invoice.date|invprefix:$prefix}{$invoice.id}{/if}</a>
			  {if isset($aInvoiceDatas.$invoiceId) && $aInvoiceDatas.$invoiceId.invoice_number}<br />ใบกำกับภาษี {$aInvoiceDatas.$invoiceId.invoice_number}{/if}
			  </td>
