...
{if $credit_available && $invoice.status=='Unpaid' && $isEnableCreditAvailable}
...
{/if}
    ...
    <form action="" method="post" style="margin-bottom:0px;">
            <select name="paymethod" onchange="submit()" style="width:100%" class="paymethod">
                ...
                {foreach from=$gateways2 key=gatewayid item=paymethod}
                 {if $gatewayid != 196}
            ...
            {securitytoken}</form>
        <div id="gateway" class="well well-small" style="display:block; text-align:left;" >
            {$gatewayDesc}
            {*$gateway*}
        </div>
    {/if}
        ...
        <div id="mailingAddressInfo">
            <br />
            <br />
            <span class="pelem"><i class="icon-tag"></i> <b>{$lang.mailingAddressTitle}</b></span>
            
            <div class="well well-small" style="display:block;">
            {if $invoice.client.mailingaddress==''}
                {$lang.mailingAsClientAddress}
            {else}
                {$invoice.client.mailingaddress|nl2br}
            {/if}
            </div>
        ...
        {if $invoice.status=='Unpaid'}
        <div>
            {assign var="httpqr" value=$system_url}
            {assign var="invoiceId" value=$invoice.id}
            {php}
                $httpqr = $this->get_template_vars('httpqr');
                $invoiceId = $this->get_template_vars('invoiceId');
                $part = MAINDIR . "qrPaymentTag30/invoice_".$invoiceId.".png";
                if(file_exists($part)){
                    echo '<p><b>QR Payment (Tag 30)</b></p>';
                    echo "<img src=\"".$httpqr."qrPaymentTag30/invoice_".$invoiceId.".png\" />";
                }
