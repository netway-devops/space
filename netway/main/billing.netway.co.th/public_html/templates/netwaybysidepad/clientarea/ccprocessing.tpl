...
{literal}
        <script>
            function ccsubmit() {
            	var invid = $('#frm_submit').find('input[name="invoice_id"]').val();
                if ($('#frm_submit').find('input[name="cc_details"]:checked').val()== 'new') {
                    if ($('#cardholder').val() == '') {
                        $('.alert-error').show().append('กรุณากรอก Card Holder Name<br />');
                        return false;
                    } else if ($('#cardnum').val() == '') {
                        $('.alert-error').show().append('กรุณากรอก Card Number<br />');
                        return false;
                    } else if ($('#expirymonth').val() == '' || $('#expiryyear').val() == '') {
                        $('.alert-error').show().append('กรุณากรอก Expiry Date<br />');
                        return false;
                    } else if ($('#cvv').val() == '') {
                        $('.alert-error').show().append('กรุณากรอก CVV/CVC2 Number');
                        return false;
                    } else {
                        $.post("?cmd=cc_modify&action=updatecc&inv="+invid+"&cardholder="+$('#cardholder').val()+"&cardcvv="+$('#cvv').val(), false, function(data){
                            $('#ccpcontent').addLoader();
                             return true;
                        });
                        ...
                        $.post("?cmd=cc_modify&action=updatecc&inv="+invid+"&cardcvv="+$('#cvv').val(), false, function(data){
                            $('#ccpcontent').addLoader();
							return true;
                        });
                       alert('send data complete');
            ...
            function rvChkCredit(){
                if ($('#frm_submit').find('input[name="cc_details"]:checked').val() == 'new') {
                    if ($('#cardholder').val() == '') {
                        $('.alert-error').show().append('กรุณากรอก Card Holder Name');
                    }
                }
            }
        </script>{/literal}
{/if}
 <form action='' method='post' id="frm_submit"onsubmit="return ccsubmit(this)">
...
<div class="wbox">
	...
	<table  width="100%" cellpadding="0" cellspacing="0"  class="table table-striped">
			...
			<tbody id="newcard" {if $ccard}style="display:none"{/if}>
                ...
                <tr><td >Card Holder Name</td><td><input type="text" name="cardholder"id="cardholder" size="25" /></td></tr>
                <tr><td align="right">{$lang.ccardnum}</td><td><input type="text" name="cc[cardnum]" id="cardnum"size="25" maxlenght="16"  class="styled"  autocomplete="off"/></td></tr>
                <tr><td align="right">{$lang.ccardexp}</td>
                    <td><input type="text" name="cc[expirymonth]" id="expirymonth"size="2" maxlength="2" class="styled span1" />/
                    <input type="text" name="cc[expiryyear]" id="expiryyear"size="2" maxlength="2"  class="styled span1"/> (MM/YY)</td></tr>
                <!--<tr><td align="right">{$lang.autosave_cc}</td><td align="left"><input type="checkbox" value="1" name="save_cc" {if !$ccard}checked="checked"{/if} /> {$lang.autosave_cc_desc}</td></tr>-->
                        ...
                        <tr><td align="right" width="160">{$lang.ccarcvv}</td><td align="left"><input type="text" id="cvv"name="cvv" size="4"  class="styled" autocomplete="off" style="width:60px"/> <a href="#newccdetails" data-toggle="modal">?</a></td></tr>
