...
class zabbix_controller extends HBController {
	...
	public function doViewIpVps($request) {
	    $this->loader->component('template/apiresponse', 'json');
        $this->json->assign("aResponse", HostbillCommon::singleton()->doViewIpVps($request));
        $this->json->show();
	}
    ...
    public function doViewIpamVPS($request) {
        $this->loader->component('template/apiresponse', 'json');
        $this->json->assign("aResponse", HostbillCommon::singleton()->doViewIpamVPS($request));
        $this->json->show();
    }
    ...
    public function doViewIpamZabbix($request) {
        $this->loader->component('template/apiresponse', 'json');
        $this->json->assign("aResponse", HostbillCommon::singleton()->doViewIpamZabbix($request));
        $this->json->show();
    }
	...
	public function doSetupIpamZabbix($request) {
	    $this->loader->component('template/apiresponse', 'json');
        $this->json->assign("aResponse", HostbillCommon::singleton()->doSetupIpamZabbix($request));
        $this->json->show();
	}
    ...
    public function doRecreate($request) {	
    	HostbillCommon::singleton()->doRecreate($request);
    }
    ...
    public function accountdetails($params) { 
        ...
        /*
        // FIX BUG FROM HOSTBILL Order Pages "VPS Hosting" เงื่อนไขนี้พี่ซ้งให้แก้ไขที่ Database แบบ Manual ครับ
        // เนื่องจากตอนกด save ที่ provision แล้ว main ip ไม่ save เกิดจากตัว hostbill มันทำการ update database ทั้งๆที่ยังไม่เคยมีการ insert ข้อมูล
        if (isset($params["account"]["ptype"]) && $params["account"]["ptype"] != '' && $params["account"]["ptype"] == 'Dedicated') {
            if (HostbillDao::singleton()->findVpsDetailsByAccountId($params["account"]["id"]) === true) {
            } else {
                HostbillDao::singleton()->insertVpsDetails($params["account"]["id"]);
            }
        }
        */
        
    	 // Assing Button Re-Create (Admin Provision Re-Create)
    	$this->template->assign("allowrecreatezabbix", true);
    	    	
    	
        // Validate Dispaly Tabs Provision
    	if (isset($params["account"]["status"]) && strtolower($params["account"]["status"]) == "active") {
    		
    		// Displays Tabs Bandwidth Provision
            $this->template->assign("rvbandwidthtab", APPDIR_MODULES . 'Hosting/zabbix/templates/AdminBandwidth.tpl');

            // Displays Tabs Ping Provision
            $this->template->assign("rvmonitortab", APPDIR_MODULES . 'Hosting/zabbix/templates/AdminMonitor.tpl');
...
/*
// DEBUG
$aServer = HostbillApi::singleton()->getServerDetailsByServerId($params["account"]["server_id"]);
$serverHostname = (isset($aServer["server"]["host"]) && $aServer["server"]["host"] != "") ? $aServer["server"]["host"] : "";
$serverUsername = (isset($aServer["server"]["username"]) && $aServer["server"]["username"] != "") ? $aServer["server"]["username"] : "";
$serverPassword = (isset($aServer["server"]["password"]) && $aServer["server"]["password"] != "") ? $aServer["server"]["password"] : "";
...
try {
    ...
    echo "<PRE>";
    print_r($api->rvcustom_rvtest(array()));
...
/*    	
        ...
        $aViewDiscoveryIp = array();
        $aViewDiscovery = HostbillCommon::singleton()->viewDiscovery(array('accountId' => $accountId, 'serverId' => $serverId));
        if (isset($aViewDiscovery["discovery"]["0"]["ip"]) && $aViewDiscovery["discovery"]["0"]["ip"] != "") {
            $aViewDiscoveryIp = HostbillCommon::singleton()->doViewDiscoveryIp(
                array(
                            'accountId' => $accountId, 
                            'serverId' => $serverId, 
                            'ip' => $aViewDiscovery["discovery"]["0"]["ip"]
                )
           );
        }
        $this->template->assign("aViewDiscoveryIp", $aViewDiscoveryIp);
        $this->template->assign("aViewDiscoveryDelay", HostbillCommon::singleton()->getSelectonDiscoveryDelay());
        ...
        $this->template->assign("stutusFreeMonitor", strtolower(HostbillApi::singleton()->getAddonFreeMonitorStatus($accountId)));
