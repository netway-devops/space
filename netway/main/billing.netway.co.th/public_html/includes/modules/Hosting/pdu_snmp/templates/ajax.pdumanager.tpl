{literal}
    <script type="text/javascript">
        var pdu_port_type = '{/literal}{$type}{literal}';
        function load_pdu_ports(select) {
            var v = $(select).val();
            if (!v)
                return;
            ajax_update('?cmd=pdu_snmp&action=loadports', {item_id: v, account_id: $('#account_id').val(), type:pdu_port_type}, '#pdu_port_loader', true);
        }
        function assignPort() {
            if (!$('#pdu_item_id').val())
                return false;
...
<div id="add_pdu" style="display:none" class="p6">
    <div class="row">
        ...
        <div class="col-md-3">
            <select name="item_id" id="pdu_item_id" class="form-control" onchange="load_pdu_ports(this)" style="margin-right:10px;">
        ...
        <div class="col-md-6" id="pdu_port_loader">
...
{if $items}
    ...
    <ul style="border:solid 1px #ddd;border-bottom:none;margin-bottom:15px" id="grab-sorter">
        {foreach from=$items item=itm}
            <li style="background:#ffffff" class="power_row"  data-item-id="{$itm.item_id}" data-port-id="{if $itm.connected}{$itm.connected.port_id}{else}{$itm.port_id}{/if}" data-id="{if $itm.connected}{$itm.connected.id}{else}{$itm.id}{/if}" >
                            ...
                            <tr>
                                ...
                                <td width="200" valign="top">
                                    <div style="padding:10px 0px;">
                                        <a onclick="return cyclePower('{$itm.item_id}', '{if $itm.connected}{$itm.connected.port_id}{else}{$itm.port_id}{/if}', '{if $itm.connected}{$itm.connected.id}{else}{$itm.id}{/if}')" class="sorter-ha btn btn-sm btn-primary" href="#"><span class="fa fa-plug"></span> On / Off</a>
                                        {if !$type}<a onclick="return unassignPort('{$itm.item_id}', '{$itm.port_id}', '{$itm.id}')" title="delete" class="btn btn-sm btn-danger" href="#"><span class="fa fa-times"></span> Unassign</a>{/if}
                                    </div>
                                </td>
                                <td>
...
<a onclick="$(this).hide();
        $('#add_pdu').show();
        return false;" class="btn btn-success" href="#"><span class="addsth"><strong>Connect PDU port to this server</strong></span></a>
{/if}
{elseif !$type}
    {if $pdus}
...
<script>loadStatuses(); loadExtras();</script>