{if $bandwidth}
<h1 style="font-size:20px">Limit: {$bandwidth.formatted.limit}, Used: {$bandwidth.formatted.used}{if $bandwidth.over}, <b style="color:red">Overage: {$bandwidth.formatted.overage}</b>{/if} </h1>

<table class="whitetable" width="100%" cellspacing="0" cellpadding="6">
    <tr>
        <td width="200" align="right">Billing method:</td>
        <td width="300"><b>{if $bandwidth.model=='total'}Total transfer{elseif $bandwidth.model=='average'}Average{else}95th percentile{/if}</b></td>
