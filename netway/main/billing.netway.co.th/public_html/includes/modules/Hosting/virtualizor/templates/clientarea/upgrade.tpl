    ...
    {if $fieldupgrades}
        ...
        <form action="" method="post">
            ...
            <table border="0" cellspacing="0" cellpadding="0" width="100%" class="ttable">
                ...
                <tr>
                    <td colspan="3" align="right" style="border-bottom:none;background:none;"><input type="submit"
                                                                                                     value="{$lang.continue}"
                                                                                                     class="blue"/></td>
                </tr>
            </table> {foreach from=$fieldupgrades item=cf key=k}<input type="hidden" name="fupgrade[{$k}][old_qty]"
                                                                       value="{$cf.qty}" />
                {foreach from=$cf.data item=it key=kk}<input type="hidden" name="fupgrade[{$k}][old_config_id]"
                                                             value="{$kk}" />{/foreach}{/foreach}
            {securitytoken}</form>
    {/if}
                                    ...
                                    <div {if $k!=0}style="display:none"{/if} class="up_desc">
                                        ...
                                        {else}
                                            ...
                                            <select name="cycle[{$i.id}]">
                                                {if $i.h!=0}
                                                    <option value="h"
                                                            {if $i.cycle=='h'}selected="selected"{/if}>{$i.h|price:$currency:true:true} {$lang.h}</option>{/if}
                                                {if $i.d!=0}
                                                    <option value="d"
                                                            {if $i.cycle=='d'}selected="selected"{/if}>{$i.d|price:$currency:true:true} {$lang.d}</option>{/if}
                                                {if $i.w!=0}
                                                    <option value="w"
                                                            {if $i.cycle=='w'}selected="selected"{/if}>{$i.w|price:$currency:true:true} {$lang.w}</option>{/if}
                                                {if $i.m!=0}
                                                    <option value="m"
                                                            {if $i.cycle=='m'}selected="selected"{/if}>{$i.m|price:$currency:true:true} {$lang.m}</option>{/if}
                                                {if $i.q!=0}
                                                    <option value="q"
                                                            {if $i.cycle=='q'}selected="selected"{/if}>{$i.q|price:$currency:true:true} {$lang.q}</option>{/if}
                                                {if $i.s!=0}
                                                    <option value="s"
                                                            {if $i.cycle=='s'}selected="selected"{/if}>{$i.s|price:$currency:true:true} {$lang.s}</option>{/if}
                                                {if $i.a!=0}
                                                    <option value="a"
                                                            {if $i.cycle=='a'}selected="selected"{/if}>{$i.a|price:$currency:true:true} {$lang.a}</option>{/if}
                                                {if $i.b!=0}
                                                    <option value="b"
                                                            {if $i.cycle=='b'}selected="selected"{/if}>{$i.b|price:$currency:true:true} {$lang.b}</option>{/if}
                                                {if $i.t!=0}
                                                    <option value="t"
                                                            {if $i.cycle=='t'}selected="selected"{/if}>{$i.t|price:$currency:true:true} {$lang.t}</option>{/if}
                                            </select>
                                        {/if}
                                    </div>
                                {/foreach}
