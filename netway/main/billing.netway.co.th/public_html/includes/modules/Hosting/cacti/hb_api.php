 ...
 * Copyright (c) 2010-2021 HostBill All Rights Reserved.
 *
...
class HB_CactiAPI {
     ...
     * @return boolean
     */
    public function verifyLogin($hash,$hash2='') {
        $hash = preg_replace("/[^a-zA-Z0-9=]/", "", $hash);

        $hash2 = explode('_',$hash2,2);

        if (function_exists('password_verify') && $hash2) {
            $user = db_fetch_assoc_prepared("SELECT username, lastfail, failed_attempts, locked, password FROM user_auth WHERE MD5(MD5(`username`))  = ? AND realm = 0 AND enabled = 'on'", array($hash2[0]));
            if($user && password_verify(base64_decode($hash2[1]),$user[0]['password'])) {
                return true;
            }

        }

        $id = db_fetch_cell("SELECT id FROM user_auth WHERE realm=0 AND  MD5(CONCAT(username,password)) = '" . $hash . "' LIMIT 1");
        if (!$id)
            return false;
        return true;
    ...
    public function getGraphImage($switch, $port, $data=array()) {
        ...
        if (!empty($data["from"]) && is_numeric( $data["from"])) {
            $graph_data_array["graph_start"] = $data["from"];
        }

        /* override: graph end time (unix time) */
        if (!empty($data["to"]) &&  is_numeric( $data["to"])) {
    ...
    public function getGraphData($switch, $port, $from, $to) {
        ...
        return rrdtool_function_xport($gid, null, array("graph_start" => $from, "graph_end" => $to, "export_csv"=>true), $xport_meta);
    }
...
class HB_CactiAPI_Response {
    ...
    public static function processRequest($request) {
        ...
        } elseif (!$api->verifyLogin($request['hash'],$request['hash2'])) {
