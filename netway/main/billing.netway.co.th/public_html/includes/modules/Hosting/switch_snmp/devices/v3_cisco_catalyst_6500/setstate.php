...
$snmp = $wrapper->connect($app['ip'], 161, $app['write'], 3, $app['security']);

if ($state) {
    $return = $snmp->set('.1.3.6.1.2.1.2.2.1.7.' . $port, 'i', 1);
} else {
    $return = $snmp->set('.1.3.6.1.2.1.2.2.1.7.' . $port, 'i', 2);
}

if (!$return) {
    throw new Exception('Unable to set new switch state');
}