...
/**
 * Tested on: Observium  19.8.10001
 */
...
class HB_ObserviumAPI {
     ...
     * @return boolean
     */
    public function verifyLogin($hash, $auth) {
        global $config;
        $hash = preg_replace("/[^a-zA-Z0-9_]/", "", $hash);
        $hash = explode('_',$hash,2);

        if ($config['auth_mechanism'] == 'ldap') {
            $auth = preg_replace("/[^a-zA-Z0-9_]/", "", $auth);
            $auth = explode('_',$auth,2);

            $data = explode('_', base64_decode($auth[1]), 2);
            $username = openssl_decrypt(base64_decode($auth[0]), $data[1], md5(base64_decode($hash[1])), 0, $data[0]);

            if (file_exists("includes/authentication/ldap.inc.php"))
                include_once("includes/authentication/ldap.inc.php");
            return ldap_authenticate($username, base64_decode($hash[1]));
        }

        $id = dbFetchRow("SELECT password,user_id FROM users WHERE MD5(MD5(`username`)) = '" . $hash[0] . "' LIMIT 1");
        if (!$id)
            return false;
        if (password_verify(base64_decode($hash[1]),$id['password'])){
            return true;
        }
        return false;
    }
...
class HB_ObserviumAPI_Response {
    ...
    public static function processRequest($request) {
        ...
        if (!isset($request['hash'])) {
        ...
        } elseif (!$api->verifyLogin($request['hash'], $request['auth'])) {
            ...
            switch ($request['cmd']) {
                ...
                case 'getgraphdata':
                    self::$output['data'] = $api->getGraphData($request['switch'], $request['port'], $request['from'], $request['to'], false);
                    break;
