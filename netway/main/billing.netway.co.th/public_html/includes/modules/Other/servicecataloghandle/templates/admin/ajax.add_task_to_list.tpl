<div id="formcontainer">
        ...
        <tr>
            ...
            <td class="conv_content form"  valign="top">
                ...
                <div class="tabb">
                    ...
                    <p id="ptDelete"><a class="new_control editbtn" href="#" onclick="{literal}if (confirm('Confirm ?')) {return deleteFulfillmentTask($(this)); return false;};{/literal}"><span>Delete</span></a></p>                    
    ...
    function lsitFulfillmentTask (oDatas)
    {
        ...
        $.each(oDatas, function (index, oData) {
            $('#listFulfillmentTask').append('<tr class="havecontrols listReplyTemplate" style=" padding: 5px; border-bottom:solid 1px #ddd;">'
                + '<td width="100">'
                ...
                + '</td>'
                + '<td><div style="display:block; width:600px; overflow-x: scroll;">'
                + '<!--'+(oData.scTitle ? oData.scTitle : '')+' &gt; '+(oData.pgName ? oData.pgName : '')+' &gt; --><strong>'+oData.name+'</strong><br />'
                + ''+oData.detail+'<br />'
                + '<div></td>'
                + '</tr>');
        });
    ...
    function viewFulfillmentTask (id)
    {
        ...
        $.getJSON('?cmd=servicecataloghandle&action=viewFulfillmentTask&id='+ id, function (a) {
            ...
            if (typeof oData.isDeleteAble !== 'undefined' && oData.isDeleteAble) {
                $('#ptDelete a').attr('title',oData.id);
            }
    ...
    function deleteFulfillmentTask (obj)
    {
        $('.spinner').show();
        var taskId  = obj.attr('title');
        $.post('?cmd=servicecataloghandle&action=deleteFulfillmentProcessTask', {
            taskId  : taskId,
            }, function (a) {
            parse_response(a);
            $('.spinner').hide();
