...
<div id="formcontainer">
    <div id="formloader">
        ...
        <tr>
            ...
            <td class="conv_content form"  valign="top">
                ...
                <div class="tabb">
                    <h3>เรียงลำดับ Fulfillment Process Group</h3>
                    
                    <!-- Start content -->
                    
                    <p style="margin-bottom:0px;">ลากเพื่อจัดลำดับการแสดงผลก่อนหลัง</p>
                    
                    <h4>{if $aGroups|@count}จัดลำดับหมวด{/if}</h4>
                    <ul id="sortGroup" class="sortableItem">
                        {if count($aGroups)}
                        {foreach from=$aGroups item=aGroup}
                        <li value="{$aGroup.id}">#{$aGroup.id} {$aGroup.name}</li>
                        {/foreach}
                        {/if}
                    </ul>
    ...
    $(document).ready( function () {
        ...
        var catLength       = {/literal}{$aGroup|@count}{literal};
        $('#sortGroup').sortable().bind('sortupdate', function() {
            $('.spinner').show();
            var sorts   = '0';
            $('#sortGroup li').each( function (i) {
                if (i == catLength) {
                    return false;
                }
                sorts   += ','+ $(this).attr('value');
            });
            $.post('?cmd=servicecataloghandle&action=orderProcessGroup', {
