...
<div id="formcontainer">
    <div id="formloader">
        ...
        <tr>
            ...
            <td class="conv_content form"  valign="top">
                ...
                <div class="tabb">
                    <h3>เรียงลำดับ {if $type == 'incidentKB'}Incident KB{else}Service Catalog{/if} </h3>
                    ...
                    <ul id="sortLists" class="sortableItem">
                        {if count($aLists)}
                        {foreach from=$aLists item=aList}
                        <li value="{$aList.id}">{$aList.title}</li>
                        {/foreach}
                        {/if}
                    </ul>
    ...
    $(document).ready( function () {
        ...
        $('#sortLists').sortable().bind('sortupdate', function() {
            $('.spinner').show();
            var sorts   = '0';
            $('#sortLists li').each( function (i) {
                if (i == catLength) {
                    return false;
                }
                sorts   += ','+ $(this).attr('value');
            });
            $.post('?cmd=servicecataloghandle&action=orderServiceCatalog', {
