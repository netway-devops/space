...
{if $results.Items}
	...
	{foreach from=$results.Items item=r}
		<li class="result"><a href="?cmd=dedimgr&do=rack&rack_id={$r.rack_id}&expand={$r.id}">{$r.typename} - {$r.label}
...
<li class="choices-header choices-Box">Racks</li>
	{foreach from=$results.Racks item=r}
		<li class="result"><a href="?cmd=dedimgr&do=rack&rack_id={$r.id}">{$r.name} ({$r.units})
...
<li class="choices-header choices-Items">Item Ports</li>
	{foreach from=$results.Ports item=r}
		<li class="result"><a href="?cmd=dedimgr&do=rack&rack_id={$r.rack_id}&expand={$r.id}">{$r.typename} - {$r.label}, port: {$r.number}
                        <span class="second">Colo: {$r.coloname}, floor: {$r.floorname}, rack: {$r.rackname}</span></a>
		</li>
	{/foreach}
{/if}
