...
//                                       , $source_content);
    ...
    function _compile_include_tag($tag_args)
    {
        ...
        if (!empty($tpl_config)) {
            $include_file_new = HBEventManager::notify('before_smartyinclude', str_replace(['\'', '"'], '', $include_file));
            if ($include_file_new[0] && is_string($include_file_new[0])) {
                $include_file = in_array($include_file[0], ['"', '\''], true) ?
                    '"' . $include_file_new[0] . '"' : $include_file_new[0];
            }
        }
