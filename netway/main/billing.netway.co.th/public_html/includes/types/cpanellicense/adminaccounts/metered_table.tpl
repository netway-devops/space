            ...
            {if $val.type=='Report'}
                <tr class="{if $smarty.foreach.usageloop.iteration%2==0}even{else}odd{/if}">
                    ...
                    {foreach from=$metered_usage_log.variables item=variab key=v}
                        <td>{if $val.$v}{$val.$v}{else}0{/if} {$variab.unit_name}(s)</td>
                    {/foreach}
    ...
    {else}
        {foreach from=$metered_usage_log.usage key=da item=val name=usageloop}
            <tr class="{if $smarty.foreach.usageloop.iteration%2==0}even{else}odd{/if}">
                <td>{$val.date}</td>
                {foreach from=$metered_usage_log.variables item=variab key=v}
                    <td>Avg: {$val[$v].use} {$variab.unit_name}<br>
                        <small>Price: {$val[$v].price|price:$metered_usage_log.currency:true:false:true:4}</small>
                    </td>
                {/foreach}
